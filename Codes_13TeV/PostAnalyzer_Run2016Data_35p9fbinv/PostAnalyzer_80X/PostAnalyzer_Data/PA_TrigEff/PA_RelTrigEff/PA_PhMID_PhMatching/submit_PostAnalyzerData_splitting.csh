...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(TrigPhotons.size() > 0 && JetId(j, "loose") && (*jetEta)[j] < Cut_Jet_eta)
      ...
      if(PassHLTDeno){
	for(Int_t i = 0; i < TrigPhotons.size(); i++){
	  if(PhoTrigObjMatching(TrigPhotons[i], PhodenoTrigObjs)){
	    h_TrigPhotonPt[0]->Fill((*phoEt)[TrigPhotons[i]]);
	    if(PassHLTNum){
	      if(PhoTrigObjMatching(TrigPhotons[i], PhoTrigObjs)){
		h_TrigPhotonPt[1]->Fill((*phoEt)[TrigPhotons[i]]);
	      }
	    }
	  }
	}
...
public :
   ...
   //trigger bits for photon matching
   const UInt_t hltEG75HEFilter = 4;
   const UInt_t hltEG90HEFilter = 5;
   const UInt_t hltEG120HEFilter = 6;
