...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   HLT_deno.push_back(HLT_IsoMu24_v);

   //Trigger bits for muon matching
   vector<UInt_t> MuTrigObjs;
   MuTrigObjs.push_back(hltFilter_isomu24);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	for(int k = 0; k < nMu; k++){
	  if(MuonId(k, muLoose)){
	    if(MuTrigObjMatching(k, MuTrigObjs)){
...
public :
   ...
   //Unprescaled Muon Triggers (Bits assigned according to ggAnalysis/ggNtuplizer/plugins/ggNtuplizer_globalEvent.cc)
   const ULong64_t HLT_Mu50_v = 21;
   const ULong64_t HLT_IsoMu24_v = 19;

   //trigger bits for muon matching
   const UInt_t hltFilter_isomu24 = 1;
   ...
   virtual Bool_t   MuTrigObjMatching(Int_t imu, vector<UInt_t> muTrigBits);
...
Bool_t PostAnalyzer_Data::MuTrigObjMatching(Int_t imu, vector<UInt_t> muTrigBits){

  int count = 0;
  bool matched = false;
  for(Int_t i = 0; i < muTrigBits.size(); i++){
    if((*muFiredTrgs)[imu]>>(muTrigBits[i]) & 1){
      count++;
    }
  }
  if(count > 0) matched = true;
