...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	for(Int_t ii = 0; ii < TrigPhotons.size(); ii++){
      ...
      if(PassHLTDeno){
	for(int k = 0; k < nMu; k++){
	  if(MuonId(k, muHighPt) && (*muPt)[k] > 53.0 && (*muEta)[k] < 2.4){
	    for(Int_t ii = 0; ii < TrigPhotons.size(); ii++){
...
// used to generate this class and read the Tree.
   if (tree == 0) {
     ...
     // chain->Add("/eos/uscms/store/user/leptonjets/varun/13TeV/Ntuples/80X/Data/ReMiniAOD03Feb2017/SingleMuonRun2016Bv2/Data_SingleMuonRun2016Bv2_1.root/ggNtuplizer/EventTree");
