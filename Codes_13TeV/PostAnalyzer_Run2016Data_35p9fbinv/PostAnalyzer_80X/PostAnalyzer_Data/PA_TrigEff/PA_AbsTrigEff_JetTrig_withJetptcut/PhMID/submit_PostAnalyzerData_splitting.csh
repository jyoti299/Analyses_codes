...
setenv DatanTuplesDir root://cmseos.fnal.gov//eos/uscms/store/user/leptonjets/varun/13TeV/Ntuples/80X/Data/JetHT/

foreach i (JetHT_Run2016B_23Sep2016v3  JetHT_Run2016C_23Sep2016v1  JetHT_Run2016D_23Sep2016v1  JetHT_Run2016E_23Sep2016v1  JetHT_Run2016F_23Sep2016v1  JetHT_Run2016G_23Sep2016v1  JetHT_Run2016H_PromptRecov2  JetHT_Run2016H_PromptRecov3)
#foreach i (JetHT_Run2016H_PromptRecov3)
...
if( ${i} == "JetHT_Run2016H_PromptRecov3" ) then
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(CutBasedPhotonID(i, "medium") && fabs((*phoSCEta)[i]) < Cut_Photon_eta) TrigPhotons.push_back(i);
      }

      TrigJets.clear();
      for(Int_t j = 0; j < nJet; j++){
	if(TrigPhotons.size() > 0 && JetId(j, "tight") && (*jetEta)[j] < Cut_Jet_eta && (*jetPt)[j] > 200)
	...
	if(PassHLTDeno){
	  for(Int_t j = 0; j < TrigJets.size(); j++){
	    if(JetTrigObjMatching(TrigJets[j], JetTrigObjs)){
	      for(Int_t ii = 0; ii < TrigPhotons.size(); ii++){
		if(GetdR((*phoSCEta)[TrigPhotons[ii]], (*jetEta)[TrigJets[j]], (*phoSCPhi)[TrigPhotons[ii]], (*jetPhi)[TrigJets[j]]) > 0.7){
		  h_TrigPhotonPt[0]->Fill((*phoEt)[TrigPhotons[ii]]);
		  h_TrigInvtmass[0]->Fill(GetInvtMass(TrigPhotons[ii], TrigJets[j]));
		  if(PassHLTNum){
		    h_TrigPhotonPt[1]->Fill((*phoEt)[TrigPhotons[ii]]);
		    h_TrigInvtmass[1]->Fill(GetInvtMass(TrigPhotons[ii], TrigJets[j]));
		  }
		}
	      }	      
	      break;
...
void PostAnalyzer_Data::BookHistograms(){
  ...
  */
  ...
  const Int_t nTrigMassBins = 27;
  const Double_t TrigMassBins[nTrigMassBins+1] = {0, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 2000, 2500, 3000, 4000, 5000};
  ...
  for(Int_t hist1 =0; hist1 < 2; ++hist1){
    ...
    sprintf(name, "h_TrigInvtmass_%s",cut1[hist1].c_str());
    h_TrigInvtmass[hist1] = new TH1F(name, "Mass of gamma+Jet", nTrigMassBins, TrigMassBins);
    h_TrigInvtmass[hist1]->GetYaxis()->SetTitle("");   h_TrigInvtmass[hist1]->GetYaxis()->CenterTitle();
    h_TrigInvtmass[hist1]->GetXaxis()->SetTitle("M_{#gammaj} (GeV)");  h_TrigInvtmass[hist1]->GetXaxis()->CenterTitle();
    h_TrigInvtmass[hist1]->Sumw2();
