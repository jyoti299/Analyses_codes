...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  //List of single photon triggers in data (Listed here just for information, not used anywhere in the study)
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	h_MetByPhPt[0]              ->Fill(pfMET/(*phoEt)[0]);
		  ...
		  if(Pass_JetEta){
		    h_CutFlow_qstar->Fill(7.5);
 		    ...
 		    if(Pass_GJdPhi){
		      ...
		      h_PToverM_BeforeCut->Fill((*phoEt)[PC]/(GetInvtMass(PC, JC)));

		      if((*phoEt)[PC]/(GetInvtMass(PC, JC)) > 0.3){
			h_CutFlow_qstar->Fill(9.5);
			
			h_PToverM_AfterCut->Fill((*phoEt)[PC]/(GetInvtMass(PC, JC)));
			...
			h_PFMetVsGJmass[1]          ->Fill(GetInvtMass(PC, JC), pfMET);
		      	...
		      	h_MetByPhPt[1]              ->Fill(pfMET/(*phoEt)[PC]);
			...
			if(Pass_GJInvtMass){
			  ...
			  h_MetByPhPt[2]              ->Fill(pfMET/(*phoEt)[PC]);
		...
		if(Pass_JetPt){
		  ...
		  if(Pass_JetEta){
			...
			if(Pass_GJdEta){
			  ...
			  h_MetByPhPt[3]              ->Fill(pfMET/(*phoEt)[PC]);
			  ...
			  if(Pass_GJInvtMass){
			    ...
			    h_MetByPhPt[4]              ->Fill(pfMET/(*phoEt)[PC]);
			...
			if(Pass_GJdEta){
			  ...
			  h_MetByPhPt[5]              ->Fill(pfMET/(*phoEt)[PC]);
			  ...
			  if(Pass_GJInvtMass){
			    ...
			    h_MetByPhPt[6]              ->Fill(pfMET/(*phoEt)[PC]);
...
void PostAnalyzer_Data::BookHistograms(){
  ...
  */
  ...
  h_PhSeedTime = new TH1F("h_PhSeedTime", "h_PhSeedTime", 100, 0, 10);
  h_PhSeedTime->GetYaxis()->SetTitle("Events");    h_PhSeedTime->GetYaxis()->CenterTitle();
  h_PhSeedTime->GetXaxis()->SetTitle("SeedTime");      h_PhSeedTime->GetXaxis()->CenterTitle();
  h_PhSeedTime->Sumw2();
