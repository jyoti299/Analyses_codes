...
foreach i (QstarToGJ_M2000_f1p0 )
set XS = (5.213E-1 ) # in pb
set totalEvents = ()
set mass_norm = ( 2000.0 )
...
foreach i (BstarToGJ_M2000_f1p0 )
set XS = (3.585E-3 ) # in pb
set totalEvents = ()
set mass_norm = (2000.0  )
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	for(Int_t ip = 0; ip <  GoodIsoPhotons.size(); ip++){
      ...
      if(PC > -1 && JC > -1) Pass_GJInvtMass = (GetInvtMass(PC, JC) >= 1500.0 && GetInvtMass(PC, JC) <= 3500.0);             
		    ...
		    h_CutFlowWt_qstar->Fill(8.5, PreBTag_EvtWt);
 		    ...
 		    if(Pass_GJdPhi){
		      ...
		      h_PToverM_BeforeCut->Fill((*phoEt)[PC]/(GetInvtMass(PC, JC)), PreBTag_EvtWt);
      		      ...
      		      if((*phoEt)[PC]/(GetInvtMass(PC, JC)) > 0.3){
...
public :
   ...
   TH1F *h_PToverM_BeforeCut;
   TH1F *h_PToverM_AfterCut;
...
void PostAnalyzer_MC::BookHistograms(){
  ...
  //For now using binning of q* 8 TeV.
   ...
   //latest binning based on q* and b* mass resolution (Summer16)
    ...
    h_PToverM_BeforeCut = new TH1F("h_PToverM_BeforeCut", "PToverM", 100, 0, 1);
    h_PToverM_BeforeCut->GetYaxis()->SetTitle("Events");    h_PToverM_BeforeCut->GetYaxis()->CenterTitle();
    h_PToverM_BeforeCut->GetXaxis()->SetTitle("P_{T}^{#gamma}/M");   h_PToverM_BeforeCut->GetXaxis()->CenterTitle();
    h_PToverM_BeforeCut->Sumw2();

    h_PToverM_AfterCut = new TH1F("h_PToverM_AfterCut", "PToverM", 100, 0, 1);
    h_PToverM_AfterCut->GetYaxis()->SetTitle("Events");    h_PToverM_AfterCut->GetYaxis()->CenterTitle();
    h_PToverM_AfterCut->GetXaxis()->SetTitle("P_{T}^{#gamma}/M");   h_PToverM_AfterCut->GetXaxis()->CenterTitle();
    h_PToverM_AfterCut->Sumw2();
