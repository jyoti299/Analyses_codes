	  ...
	  if(GoodIsoPhotons.size() > 0){
		...
		if(Pass_JetPt){
		  ...
		  if(Pass_JetEta){
			...
			//if(${GJ} && !(IsOverlappedEvent(PC))) continue;
			...
			h_PToverM_BeforeCut->Fill((*phoEt)[PC]/(GetInvtMass(PC, JC)), PreBTag_EvtWt);

			if((*phoEt)[PC]/(GetInvtMass(PC, JC)) > 0.1){
			  h_CutFlow_qstar->Fill(11.5);
			  h_CutFlowWt_qstar->Fill(11.5, PreBTag_EvtWt);
			  h_PToverM_AfterCut->Fill((*phoEt)[PC]/(GetInvtMass(PC, JC)), PreBTag_EvtWt);

			  if(Pass_GJInvtMass){
			    h_CutFlow_qstar->Fill(12.5);
			    h_CutFlowWt_qstar->Fill(12.5, PreBTag_EvtWt);
