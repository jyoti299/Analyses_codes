...
foreach case ( 0 3 6 7 8)  #0  3  6  7 )
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	for(Int_t ip = 0; ip <  GoodIsoPhotons.size(); ip++){
      ...
      if(JC > -1) Pass_LepVeto = ((!OverlapWithElectron(fabs((*jetEta)[JC]), (*jetPhi)[JC])) && (!OverlapWithMuon(fabs((*jetEta)[JC]), (*jetPhi)[JC])));
...
public :
   ...
   virtual Bool_t   CutBasedElectronID(Int_t iele, UShort_t WP);
   virtual Bool_t   MuonId(Int_t iMu, UShort_t Cut);
   virtual Bool_t   OverlapWithElectron(double jeteta, double jetphi);
   virtual Bool_t   OverlapWithMuon(double jeteta, double jetphi);
...
Bool_t PostAnalyzer_MC::CutBasedElectronID(Int_t iele, UShort_t WP){

  Bool_t eleId = false;
  if((*eleIDbit)[iele]>>WP & 1) eleId = true;
  return eleId;   

}

Bool_t PostAnalyzer_MC::MuonId(Int_t iMu, UShort_t Cut){
...
Bool_t PostAnalyzer_MC::OverlapWithElectron(double jeteta, double jetphi){

  bool overlap = false;
  for(int i = 0; i < nEle; i++){
    if(CutBasedElectronID(i, eleVeto) && (*elePt)[i] > 10.0){

      float dRJetEle = GetdR((*eleEta)[i], jeteta, (*elePhi)[i], jetphi);
      if(dRJetEle < 0.5){
        overlap = true;
        break;
      }
...
Bool_t PostAnalyzer_MC::OverlapWithMuon(double jeteta, double jetphi){

  bool overlap = false;
  for(int k = 0; k < nMu; k++){
    if(MuonId(k, muLoose) && (*muPt)[k] > 10.0){

      float dRJetMu = GetdR((*muEta)[k], jeteta, (*muPhi)[k], jetphi);
      if(dRJetMu < 0.5){
        overlap = true;
        break;
      }
...
void PostAnalyzer_MC::BookHistograms(){
  ...
  //For now using binning of q* 8 TeV.
    ...
    */
    ...
    TString CutFlowLabels_qstar[nbins_qstar] = {"Total", "HLT", "PrimaryVtx", "PhotonID", "PhotonPtEta", "PhotonPtEta_AfterOverlapRemoval", "JetID&LepVeto", "JetPt", "JetEta", "DPhi", "DEta", "MassCut"};
  TString CutFlowLabels_bstar[nbins_bstar] = {"Total", "HLT", "PrimaryVtx", "PhotonID", "PhotonPtEta", "JetID&LepVeto", "JetPt", "JetEta", "1BTag", "1BTag_DPhi", "1BTag_DEta", "1BTag_MassCut", "0BTag", "0BTag_DPhi", "0BTag_DEta", "0BTag_MassCut"};
  TString CutFlowLabelsWt_qstar[nbinsWt_qstar] = {"Total", "HLT", "PrimaryVtx", "PhotonID", "PhotonPtEta", "PhotonPtEta_AfterOverlapRemoval", "JetID&LepVeto", "JetPt", "JetEta", "DPhi", "DEta", "MassCut"};
  TString CutFlowLabelsWt_bstar[nbinsWt_bstar] = {"Total", "HLT", "PrimaryVtx", "PhotonID", "PhotonPtEta", "JetID&LepVeto", "JetPt", "JetEta", "1BTag", "1BTag_DPhi", "1BTag_DEta", "1BTag_MassCut", "0BTag", "0BTag_DPhi", "0BTag_DEta", "0BTag_MassCut"};
  TString CutFlowLabelsTotalWt_bstar[nbinsTotalWt_bstar] = {"Total", "HLT", "PrimaryVtx", "PhotonID", "PhotonPtEta", "JetID&LepVeto", "JetPt", "JetEta", "1BTag", "1BTag_DPhi", "1BTag_DEta", "1BTag_MassCut", "0BTag", "0BTag_DPhi", "0BTag_DEta", "0BTag_MassCut"};
