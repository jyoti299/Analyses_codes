...
foreach i (QstarToGJ_M500_f1p0  QstarToGJ_M1000_f1p0  QstarToGJ_M2000_f1p0  QstarToGJ_M3000_f1p0  QstarToGJ_M4000_f1p0  QstarToGJ_M5000_f1p0  QstarToGJ_M6000_f1p0  QstarToGJ_M7000_f1p0 )
set XS = (3.033E2  1.632E1  5.213E-1  4.272E-2  4.8E-3  5.835E-4  7.076E-5  8.66E-6 ) # in pb
set totalEvents = ()
#2 sigma
set mass_min = ( 400.0  700.0  1500.0  2400.0  3200.0  4000.0  4600.0  5000.0 )
set mass_max = ( 600.0  1300.0  2500.0  3600.0  4800.0  6000.0  7400.0  9000.0 )
#3 sigma
#set mass_min = ( 300.0  500.0  1300.0  2200.0  3000.0  3700.0  4200.0  4500.0 )
#set mass_max = ( 700.0  1500.0  2700.0  3800.0  5000.0  6300.0  7600.0  9500.0 )
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     //cout << "<Analyzing entry: " << jentry << endl;
      ...
      Double_t Mass_min = ${mass_min[${sampleIndex}]};
      Double_t Mass_max = ${mass_max[${sampleIndex}]};
	...
	for(Int_t ip = 0; ip <  GoodIsoPhotons.size(); ip++){
      ...
      if(PC > -1 && JC > -1) Pass_GJInvtMass = ((GetInvtMass(PC, JC)) > Mass_min && (GetInvtMass(PC, JC)) < Mass_max);             
...
Int_t PostAnalyzer_MC::MatchedGenPhotonToReco(Int_t pc){ 
  ...
  for(Int_t ij = 0; ij < nMC; ij++){
    dR = GetdR((*phoSCEta)[pc], (*mcEta)[ij], (*phoSCPhi)[pc], (*mcPhi)[ij]);
    //dPt_Pt = fabs((*phoEt)[pc] - (*mcPt)[ij])/(*phoEt)[pc];
    if((*mcPID)[ij] == 22 && dR < 0.1){ // && dPt_Pt < 0.1){
      pc_gen = ij;
      break;
    }
  }
...
void PostAnalyzer_MC::BookHistograms(){
  ...
  //For now using binning of q* 8 TeV.
   ...
   //latest binning based on q* and b* mass resolution (Summer16)
    ...
    h_JetPt = new TH1F("h_JetPt","Pt distribution of jets",300,0.0,6000.0);
    h_JetPt->GetYaxis()->SetTitle("Events/20 GeV");  h_JetPt->GetYaxis()->CenterTitle();
    h_JetPt->GetXaxis()->SetTitle("P_{T}^{Jet} (GeV)");  h_JetPt->GetXaxis()->CenterTitle();
    h_JetPt->Sumw2();

    h_GenJetPt = new TH1F("h_GenJetPt","Pt distribution of gen jets",300,0.0,6000.0);
    h_GenJetPt->GetYaxis()->SetTitle("Events/20 GeV");  h_GenJetPt->GetYaxis()->CenterTitle();
    h_GenJetPt->GetXaxis()->SetTitle("P_{T}^{Jet} (GeV)");  h_GenJetPt->GetXaxis()->CenterTitle();
    h_GenJetPt->Sumw2();

    h_GenPartonPt = new TH1F("h_GenPartonPt","Pt distribution of gen partons",300,0.0,6000.0);
    h_GenPartonPt->GetYaxis()->SetTitle("Events/20 GeV");  h_GenPartonPt->GetYaxis()->CenterTitle();
    h_GenPartonPt->GetXaxis()->SetTitle("P_{T}^{Jet} (GeV)");  h_GenPartonPt->GetXaxis()->CenterTitle();
    h_GenPartonPt->Sumw2();

    h_JetEta = new TH1F("h_JetEta","Eta distribution of jets",70,-3.5,3.5);
    ...
    h_GenJetEta = new TH1F("h_GenJetEta","Eta distribution of gen jets",70,-3.5,3.5);
    h_GenJetEta->GetYaxis()->SetTitle("Events");  h_GenJetEta->GetYaxis()->CenterTitle();
    h_GenJetEta->GetXaxis()->SetTitle("#eta^{Jet}");  h_GenJetEta->GetXaxis()->CenterTitle();
    h_GenJetEta->Sumw2();

    h_GenPartonEta = new TH1F("h_GenPartonEta","Eta distribution of gen partons",70,-3.5,3.5);
    h_GenPartonEta->GetYaxis()->SetTitle("Events");  h_GenPartonEta->GetYaxis()->CenterTitle();
    h_GenPartonEta->GetXaxis()->SetTitle("#eta^{Jet}");  h_GenPartonEta->GetXaxis()->CenterTitle();
    h_GenPartonEta->Sumw2();

    h_PhotonPt = new TH1F("h_PhotonPt","Pt distribution of photons",300,0.0,6000.0);
    h_PhotonPt->GetYaxis()->SetTitle("Events/20 GeV");  h_PhotonPt->GetYaxis()->CenterTitle();
    h_PhotonPt->GetXaxis()->SetTitle("P_{T}^{#gamma} (GeV)");  h_PhotonPt->GetXaxis()->CenterTitle();
    h_PhotonPt->Sumw2();

    h_GenPhotonPt = new TH1F("h_GenPhotonPt","Pt distribution of gen photons",300,0.0,6000.0);
    h_GenPhotonPt->GetYaxis()->SetTitle("Events/20 GeV");  h_GenPhotonPt->GetYaxis()->CenterTitle();
    h_GenPhotonPt->GetXaxis()->SetTitle("P_{T}^{#gamma} (GeV)");  h_GenPhotonPt->GetXaxis()->CenterTitle();
    h_GenPhotonPt->Sumw2();

    h_PhotonEta = new TH1F("h_PhotonEta","Eta distribution of photons",70,-3.5,3.5);
    ...
    h_GenPhotonEta = new TH1F("h_GenPhotonEta","Eta distribution of gen photons",70,-3.5,3.5);
    h_GenPhotonEta->GetYaxis()->SetTitle("Events");  h_GenPhotonEta->GetYaxis()->CenterTitle();
    h_GenPhotonEta->GetXaxis()->SetTitle("#eta^{#gamma}");  h_GenPhotonEta->GetXaxis()->CenterTitle();
    h_GenPhotonEta->Sumw2();
