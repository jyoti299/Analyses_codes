	  ...
	  if(GoodIsoPhotons.size() > 0){
		...
		if(Pass_JetPt){
		  ...
		  if(Pass_JetEta){
			...
			Int_t PC_match = MatchedGenPhotonToReco(PC); //Matched gen photon with dR < 0.1 and dpt/pt < 0.1

			Bool_t isPrompt = false;
			if((*mcStatusFlag)[PC_match] == 2 || (*mcStatusFlag)[PC_match] == 3 || (*mcStatusFlag)[PC_match] == 6){
			  double dR_j = 10.0;
			  double dR_temp_j = 10.0;
			  for(Int_t ijet = 0; ijet < nJet; ijet++){
			    dR_temp_j = GetdR((*mcEta)[PC_match], (*jetGenEta)[ijet], (*mcPhi)[PC_match], (*jetGenPhi)[ijet]);
			    if(dR_temp_j < dR_j) dR_j = dR_temp_j;
			  }
			  if(dR_j > 0.05)  isPrompt = true;
			}
			//comment in script
			//if(isPrompt) continue;
			//uncomment in script
			...
			//h_GenRecoPhoton_dR->Fill(GetdR((*phoSCEta)[PC], (*mcEta)[PC_match], (*phoSCPhi)[PC], (*mcPhi)[PC_match]), PreBTag_EvtWt);
