   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     //cout << "<Analyzing entry: " << jentry << endl;
      ...
      //      bool removeGJ_PromPh_insideDR0p4 = false;
      /*
      if(removeGJ_QCD_Overlapping){
	//Removing GJet/QCD overlapping
	bool isPrompt = false;
	//isPrompt = IsPromptFound(); //looking for prompt photon without any dR requirement between prompt photon and jets.
	isPrompt = IsPromptFoundOutOf_dR(0.05);//look for prompt photon with given dR requirement between prompt photon and jets.
	...
	//Uncomment it in script
      ...
      */
      /*
      if(removeGJ_PromPh_insideDR0p4){
	bool isPrompt_insideOfDR0p4 = false;
	isPrompt_insideOfDR0p4 = IsPromptFoundInsideOf_dR(0.4);

	//comment it in script
	if(isPrompt_insideOfDR0p4) continue;

	//Uncomment it in script
        //if(${GJ} && isPrompt_insideOfDR0p4) continue; //reject event if prompt photon is within dR 0.4 with jet in GJet sample
	  ...
	  if(GoodIsoPhotons.size() > 0){
		...
		if(Pass_JetPt){
		  ...
		  if(Pass_JetEta){
			...
			Int_t PC_match = MatchedGenPhotonToReco(PC); //Matched prompt gen photon with dR < 0.1 and dpt/pt < 0.1

			//////earlier something similar was done. just first got the matched gen photon with dR < 0.1 and dPt/pt < 0.1
			////then looked if it is prompt by looking into the mcstatuscode ==2/3/6.
			///then it is matched in dR with the njet collection for the gen parton from jet collection.
			///this is the main differnce here as here we are taking nMC collection instead of nJet.
			...
			if(PC_match > -1){ //true matched photons including overlaps
			  ...
			  h_GenRecoPhoton_dR[0]->Fill(minDR, PreBTag_EvtWt);
			  h_GenRecoPhoton_dPToverPT[0]->Fill(fabs((*phoEt)[PC] - (*mcPt)[PC_match])/(*phoEt)[PC], PreBTag_EvtWt);

			  if(minDR > 0.05)  isPrompt = true;
			}

			h_GenRecoPhoton_dR[1]->Fill(minDR, PreBTag_EvtWt);
			if(PC_match > -1) h_GenRecoPhoton_dPToverPT[1]->Fill(fabs((*phoEt)[PC] - (*mcPt)[PC_match])/(*phoEt)[PC], PreBTag_EvtWt);

			//comment in script
			//if(isPrompt) continue;
			//uncomment in script
			if(${QCD} && isPrompt) continue;

			h_GenRecoPhoton_dR[2]->Fill(minDR, PreBTag_EvtWt);
			if(PC_match > -1) h_GenRecoPhoton_dPToverPT[2]->Fill(fabs((*phoEt)[PC] - (*mcPt)[PC_match])/(*phoEt)[PC], PreBTag_EvtWt);
...
public :
   ...
   TH1F *h_GenRecoPhoton_dR[3];
   TH1F *h_GenRecoPhoton_dPToverPT[3];
...
/*
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_119.root/ggNtuplizer/EventTree");
      ...
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_123.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_125.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_167.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_154.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_147.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_135.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_134.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/Ntuples/80X/MC/Dijet/QCD_Pt300to470/MC_QCD_Pt300to470_133.root/ggNtuplizer/EventTree");
  */    
...
void PostAnalyzer_MC::BookHistograms(){
  ...
  //For now using binning of q* 8 TeV.
    ...
    */
    ...
    std::string cut1[3] = {"OnlyPrompts", "BeforeOverlapRemoval", "AfterOverlapRemoval"};
    for(Int_t hist1 = 0; hist1 < 3; ++hist1){

      sprintf(name, "h_GenRecoPhoton_dR_%s", cut1[hist1].c_str());
      h_GenRecoPhoton_dR[hist1] = new TH1F(name,"dR between Reco and Gen photon", 10000, 0.0, 100.0);   
      h_GenRecoPhoton_dR[hist1]->GetYaxis()->SetTitle("Events");   h_GenRecoPhoton_dR[hist1]->GetYaxis()->CenterTitle();
      h_GenRecoPhoton_dR[hist1]->GetXaxis()->SetTitle("#Delta R"); h_GenRecoPhoton_dR[hist1]->GetXaxis()->CenterTitle();
      h_GenRecoPhoton_dR[hist1]->Sumw2();
    
      sprintf(name, "h_GenRecoPhoton_dPToverPT_%s", cut1[hist1].c_str());
      h_GenRecoPhoton_dPToverPT[hist1] = new TH1F(name,"dPt/Pt between Reco and Gen Photon", 100, 0.0, 1.0);
      h_GenRecoPhoton_dPToverPT[hist1]->GetYaxis()->SetTitle("Events");      h_GenRecoPhoton_dPToverPT[hist1]->GetYaxis()->CenterTitle();
      h_GenRecoPhoton_dPToverPT[hist1]->GetXaxis()->SetTitle("dPt/Pt");      h_GenRecoPhoton_dPToverPT[hist1]->GetXaxis()->CenterTitle();
      h_GenRecoPhoton_dPToverPT[hist1]->Sumw2();
    ...
    TString CutFlowLabels_qstar[nbins_qstar] = {"Total", "HLT", "PrimaryVtx", "PhotonID", "PhotonPtEta", "JetID", "JetPt", "JetEta", "DPhi", "DEta", "DEta_WithOverlapRemoval", "MassCut"};
  ...
  TString CutFlowLabelsWt_qstar[nbinsWt_qstar] = {"Total", "HLT", "PrimaryVtx", "PhotonID", "PhotonPtEta", "JetID", "JetPt", "JetEta", "DPhi", "DEta", "DEta_WithOverlapRemoval", "MassCut"};
