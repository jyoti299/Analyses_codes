   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(HasPrimaryVtx){
	  ...
	  if(GoodIsoPhotons.size() > 0){
	    ...
	    Int_t PC_Num = GoodIsoPhotons[0];
	    Int_t Gen_PC_Num = MatchedGenPhotonToReco(PC_Num);

	    h_PhotonPt->Fill((*phoEt)[PC_Num], PreBTag_EvtWt);
	    h_GenPhotonPt->Fill((*mcPt)[Gen_PC_Num], PreBTag_EvtWt);
	    h_PhotonEta->Fill((*phoSCEta)[PC_Num], PreBTag_EvtWt);
	    h_GenPhotonEta->Fill((*mcEta)[Gen_PC_Num], PreBTag_EvtWt);
		...
		if(Pass_JetPt){
		  ...
		  if(Pass_JetEta){
			...
			if(fabs((*jetGenPartonID)[JC]) == 1){
			  h_JetPt->Fill((*jetPt)[JC], PreBTag_EvtWt);
			  h_GenJetPt->Fill((*jetGenJetPt)[JC], PreBTag_EvtWt);
			  h_GenPartonPt->Fill((*jetGenPt)[JC], PreBTag_EvtWt);
			  h_JetEta->Fill((*jetEta)[JC], PreBTag_EvtWt);
			  h_GenJetEta->Fill((*jetGenJetEta)[JC], PreBTag_EvtWt);
			  h_GenPartonEta->Fill((*jetGenEta)[JC], PreBTag_EvtWt);
			}
