   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	for(Int_t ip = 0; ip <  GoodIsoPhotons.size(); ip++){
      ...
      //Data/MC scale factor for trigger efficiency
      //if(PC > -1){
      //	if((*phoEt)[PC] <= 700){
      //  PreBTag_EvtWt = 0.95 * PreBTag_EvtWt;
     //	}
     // }
	...
	if(HasPrimaryVtx){
      ...
      Int_t Selectedjet = -1;
      goodjets.clear();
      goodjets = GoodJets(Cut_JetId);
      if(goodjets.size() != 0){
	for(Int_t jj = 0; jj < goodjets.size(); jj++){
	  if(fabs((*jetEta)[goodjets[jj]]) < 2.4){
...
void PostAnalyzer_MC::BookHistograms(){
  ...
  //For now using binning of q* 8 TeV.
    ...
    */
    ...
    const Int_t n_eff_pt = 22;
    const Double_t Eff_bins_pt[n_eff_pt+1] = {50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180, 190, 200, 250, 300, 400, 500, 600, 800, 1000};
