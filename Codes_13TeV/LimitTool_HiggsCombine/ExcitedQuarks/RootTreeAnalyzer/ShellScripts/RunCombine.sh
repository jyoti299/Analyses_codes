...
##for Qstar
#python python/RunCombine.py -m ${model} -d ${outdir} --mass range\(1000,6500,50\) -c ${config} -i ${inFitFile} -b ${boxName} --rMax 2 --xsec 0.01 -l 35.866 --coup ${coupling} --signif  #--partial-signal-sys --freezeNuc ,jes,jer,pes,per,bsf,trig #--deco
...
##for Bstar
#python python/RunCombine.py -m ${model} -d ${outdir} --mass 2800 -c ${config} -i ${inFitFile} -b ${boxName} --rMax 2 --xsec 0.01 -l 35.866 --coup ${coupling} #--no-signal-sys #--partial-signal-sys --freezeNuc jes,jer,pes,per,bsf,trig #--deco  
