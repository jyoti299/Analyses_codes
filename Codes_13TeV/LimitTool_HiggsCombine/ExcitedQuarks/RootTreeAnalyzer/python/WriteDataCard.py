...
def computeBkgBias(w,box,model,lumi,massPoint=0,multi=False):
    if multi:
       bkgPdf = w.pdf('%s_multi'%(box))
       rooNdata = w.var('Ntot_multi_%s'%(box))
    else:
       bkgPdf = w.pdf('%s_bkg'%(box))
       rooNdata = w.var('Ntot_bkg_%s'%(box))
    signalDataHist = w.data('%s_%s'%(box,model))
    signalPdf = rt.RooHistPdf('pdf_%s_%s'%(box,model),'pdf_%s_%s'%(box,model), rt.RooArgSet(w.var('th1x')), signalDataHist)

    roopdflist = rt.RooArgList()
    roopdflist.add(bkgPdf)
    roopdflist.add(signalPdf)        

    roolist = rt.RooArgList()
    nBias = rt.RooRealVar('nBias_bkg_%s'%(model), 'nBias_bkg_%s'%(model), 0)
    nBias.setVal(0.)
    nBias.setConstant(True)

    massMin = 0
    massMax = 6000
    bias_func = rt.TF1('Bkg_bias_func', "TMath::Min(3.1, 7.6e10*x^(-3))", massMin, massMax)
#    bias_func = rt.TF1('Bkg_bias_func', "TMath::Min(60.1, 150e10*x^(-3))", massMin, massMax)

    if massPoint < 2000:
       bias_func = rt.TF1('Bkg_bias_func', "TMath::Min(10.0, 2.4e13*x^(-4))", massMin, massMax)
    else:
       bias_func = rt.TF1('Bkg_bias_func', "TMath::Min(0.5, 2.4e13*x^(-4))", massMin, massMax)

    sig_fwhm = computeFWHM(w,box,model)
    ...
    nB = 0
#    nB = bias_func.Eval(float(massPoint)) * sig_fwhm * lumi/1000
#    nB = bias_func.Eval(float(massPoint)) * sig_fwhm
    nB = 1000

    pdfSum_norm = rt.RooFormulaVar("%s_bkg_bias_norm" %(box),"%s_bkg_bias_norm" %(box),"@0",rt.RooArgList(rooNdata)) 
    fracsignuis = rt.RooFormulaVar("signal_%s_%s_nuisanced_frac" %(box,model),"signal_%s_%s_nuisanced_frac" %(box,model),"@0*1./@1",rt.RooArgList(nBias,pdfSum_norm) )
    fracbkg = rt.RooFormulaVar("background_%s_%s_frac" % (box,model), "background_%s_%s_frac" %(box,model), "1.-@0",rt.RooArgList(fracsignuis))

    roolist.add(fracbkg)
    roolist.add(fracsignuis)

    pdfSum = rt.RooAddPdf("%s_bkg_bias" %(box),"%s_bkg_bias" %(box), roopdflist, roolist)
    rootTools.Utils.importToWS(w,pdfSum_norm)
    rootTools.Utils.importToWS(w,pdfSum,rt.RooFit.RecycleConflictNodes())
  
    return w, nBias, nB

def computeFWHM(w,box,model):

    roobs = w.var('mgj')
#    binned = w.data('%s_%s_mgj'%(box,model))
    binned = w.pdf('pdf_%s_%s_mgj'%(box,model))
    nBins = 1000
    roobs.setBins(nBins)
...
def initializeWorkspace(w,cfg,box,scaleFactor=1.,penalty=False,multi=False,x=None,emptyHist1D=None):
    ...
    if x is None:
        x = array('d', cfg.getBinning(box)[0]) # mgj binning ##as getBinning() in framework/Config.py runs over both mgj and th1x and [0] is for mgj
    
    nBins = len(x)-1
    maxBins = nBins
    
    variables = cfg.getVariablesRange(box, "variables",w) ##This by calling getVariablesRange() defines the variables from box into the workspace along with range to this variable. So it defines var mgj and th1x with ranges into the workspace.
    ...
    constPars = ['sqrts','p0_%s'%box, 'sqrts5', 'p50_%s'%box, 'sqrtsm', 'pm0_%s'%box, 'sqrtsa', 'pa0_%s'%box, 'sqrtsf1', 'sqrtsf2', 'sqrtsf3', 'sqrtsf4', 'sqrtsf5']
    ...
    for command in commands:
        ...
        if lower.find('sum::')!=-1 or lower.find('prod::')!=-1 or lower.find('expr::')!=-1 or lower.find('roogaussian::')!=-1 or lower.find('rooefficiency:')!=-1:
            ...
            altname = '_'.join(reversed(name.split('_')))  ##It will join the keywords with _ in reverse order i.e ==> a_b_c to c_b_a
            ...
            args = tuple(arglist) ###tuple of arglist 
            pdf = getattr(rt,myclass)(*args) ###rt.myclass(*args) ==> rt.RooDijetBinPdf(*args) => making RooDijetbinPdf and giving the name pdf.
            if hasattr(pdf,'setTH1Binning'): ##if setTH1Binning is one of the attributes of pdf.##defined in HiggsAnalysis/CombinedLimit/src/RooDijetBinPdf.cc and interface/.h
                pdf.setTH1Binning(emptyHist1D)
            rootTools.Utils.importToWS(w,pdf) ##import RooDijetbinPdf in workspace.
            bkg = name.split("_")
...
#                        lumiErrs = [1.500]            
        ...
        trigErrs.extend([1.00 for bkg in bkgs])
        divider = "------------------------------------------------------------\n"
...
def writeDataCard_bias(box,model,txtfileName,bkgs,paramNames,w,penalty,fixed,shapes=[],multi=False,BiasName="",BiasSigma=0):
        obsRate = w.data("data_obs").sumEntries()
        nBkgd = len(bkgs)
        rootFileName = txtfileName.replace('.txt','.root')
        signals = len(model.split('p'))
        if signals>1:
                ...
                if '2015' in box:
                        ...
                        trigErrs = [1.050]
        rates.extend([1])
        processes.extend(["%s_bkg_bias"%(box)])
...
#        datacard+=trigString
        ...
        ##adding nuisances to account for bias: BiasName will be a gaussian with mean = 0 and sigma = bias (nB computed above).
        ## Now the Background ExciteQuarks2016_bkg will be multiplied by (1-nB/Ntot_Bkg) and signal (taken as bias:ExcitedQuarks2016_bkg_bias)
        ## will be multiplied by nB/Ntot_Bkg
        datacard += "%s\tparam\t0\t%.3f\n"%(BiasName, BiasSigma)
#        datacard += "alpha\trateParam\t%s\t%s_bkg_bias\t((1.0-@0/%.3f)*%.3f+(@0/%.3f)*%.3f)\t%s\n"%(box,box,w.var('Ntot_%s_%s'%(bkg,box)).getVal(),w.var('Ntot_%s_%s'%(bkg,box)).getVal(),w.var('Ntot_%s_%s'%(bkg,box)).getVal(),rates[0],BiasName)

#        datacard += "alpha\trateParam\t%s\t%s_bkg\t(1.0-@0/%.3f)\t%s\n"%(box,box,rates[1],BiasName)
#        datacard += "beta\trateParam\t%s\t%s_bkg_bias\t(@0/%.3f)\t%s\n"%(box,box,rates[1],BiasName)

#        datacard += "%s\tparam\t0\t1\n"%(BiasName) 
#        datacard += "alpha\trateParam\t%s\t%s_bkg\t(1.0-@0*%.3f/%.3f)\t%s\n"%(box,box,BiasSigma,rates[1],BiasName)
#        datacard += "beta\trateParam\t%s\t%s_bkg_bias\t(@0*%.3f/%.3f)\t%s\n"%(box,box,BiasSigma,rates[1],BiasName)
...
def convertToMjjHist(hist_th1x,x):
    ...
    hist = rt.TH1D(hist_th1x.GetName()+'_mgj',hist_th1x.GetName()+'_mgj',len(x)-1,x)
...
def applyTurnonFunc(hist,effFr,w):
    ...
    for i in range(1,hist.GetNbinsX()+1):
        w.var('mgj').setVal(hist.GetXaxis().GetBinCenter(i))
        #print 'mgj = %f, eff = %f'%(hist.GetXaxis().GetBinCenter(i), w.function('effFunc').getVal(rt.RooArgSet(w.var('mgj'))))
        hist_turnon.SetBinContent(i,hist.GetBinContent(i)*w.function('effFunc').getVal(rt.RooArgSet(w.var('mgj'))))
...
if __name__ == '__main__':
    ...
    parser.add_option('--jerDown',dest="jerDownFile", default=None,type="string",
                  help="jer Down file")
    parser.add_option('--pesUp',dest="pesUpFile", default=None,type="string",
                  help="pes Up file")
    parser.add_option('--perUp',dest="perUpFile", default=None,type="string",
                  help="per Up file")
    parser.add_option('--pesDown',dest="pesDownFile", default=None,type="string",
                  help="pes Down file")
    parser.add_option('--perDown',dest="perDownFile", default=None,type="string",
                  help="per Down file")
    parser.add_option('--bsfUp',dest="bsfUpFile", default=None,type="string",
                  help="b-tag SF Up file")
    parser.add_option('--bsfDown',dest="bsfDownFile", default=None,type="string",
                  help="b-tag SF Down file")
    ...
    for f in args:
        if f.lower().endswith('.root'):
            if f.lower().find('resonanceshapes')!=-1:
                ...
                if 'h_mgj_13TeV_1GeVBin' in names:
                    myTH1 = rootFile.Get('h_mgj_13TeV_1GeVBin')
                elif 'mgj_gev' in names:
                    myTH1 = rootFile.Get('mgj_gev')
                    myTH1.Print('v')
    ...
    x = array('d', cfg.getBinning(box)[0]) # mgj binning
...
#                sigDataHist_bias = sigDataHist.Clone('%s_bkg_bias'%(box))
                sigDataHist_mgj = rt.RooDataHist('%s_%s_mgj'%(box,model),'%s_%s_mgj'%(box,model), rt.RooArgList(w.var('mgj')), rt.RooFit.Import(d))
                sigPdf_mgj = rt.RooHistPdf('pdf_%s_%s_mgj'%(box,model),'pdf_%s_%s_mgj'%(box,model), rt.RooArgSet(w.var('mgj')), sigDataHist_mgj)
                rootTools.Utils.importToWS(w,sigDataHist)
 #               rootTools.Utils.importToWS(w,sigDataHist_bias)
                rootTools.Utils.importToWS(w,sigDataHist_mgj)
                rootTools.Utils.importToWS(w,sigPdf_mgj)
    ...
    if options.computeBias:
       w, nBias, nB = computeBkgBias(w,box,model,lumi,massPoint,multi=options.multi)       
    ...
    if options.noSignalSys:
        ...
        if options.pesUpFile is not None or options.pesDownFile is not None:
            shapes.append('pes')
            shapeFiles['pesUp'] = options.pesUpFile
            shapeFiles['pesDown'] = options.pesDownFile
        if options.perUpFile is not None or options.perDownFile is not None:
            shapes.append('per')
            shapeFiles['perUp'] = options.perUpFile
            shapeFiles['perDown'] = options.perDownFile
    if options.bsfUpFile is not None or options.bsfDownFile is not None:
            shapes.append('bsf')
            shapeFiles['bsfUp'] = options.bsfUpFile
            shapeFiles['bsfDown'] = options.bsfDownFile

    # JES, PES, JER, PER and BSF uncertainties 
    ...
    for shape in shapes:
        if shapeFiles[shape+'Up'] is not None:
            ...
            hUpTh1x.Scale(hSigTh1x.Integral()/hUpTh1x.Integral()) ##scaling it to make the same as hSigTh1x as hSigTh1x is already scaled by lumi.
    ...
    outFile = 'ExcitedQuarks_combine_%s_%i_lumi-%.3f_%s.root'%(model,massPoint,lumi/1000.,box)
    outputFile = rt.TFile.Open(options.outDir+"/"+outFile,"recreate")
    if options.computeBias:
       writeDataCard_bias(box,model,options.outDir+"/"+outFile.replace(".root",".txt"),bkgs,paramNames,w,options.penalty,options.fixed,shapes=shapes,multi=options.multi, BiasName=nBias.GetName(), BiasSigma=nB)
    else:
