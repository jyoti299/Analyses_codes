...
def convertSideband(name,w,x):
    ...
    for ix in range(1,len(x)):
        iBinX+=1
        w.var('mgj').setVal((x[ix]+x[ix-1])/2.)
        inSideband = 0
        for fitname in names:
            inSideband += ( w.var('mgj').inRange(fitname) )
...
def convertFunctionToHisto(background_,name_,N_massBins_,massBins_):
    print "+++ Running convertFunctionToHisto +++" 
...
def calculateChi2AndFillResiduals(data_obs_TGraph_,background_hist_,hist_fit_residual_vsMass_,workspace_,prinToScreen_=0,effFit_=False):
    print "+++ Running calculateChi2AndFillResiduals +++" 
    ...
    for bin in range (0,N_massBins_):
        ...
        checkInRegions = [xbinCenter>workspace_.var('mgj').getMin(reg) and xbinCenter<workspace_.var('mgj').getMax(reg) for reg in plotRegions]
        if effFit_: checkInRegions = [xbinCenter>workspace_.var('mgj').getMin('Eff') and xbinCenter<workspace_.var('mgj').getMax('Eff')]
...
if __name__ == '__main__':
    ...
    myTH1 = None
    for f in args:
        if f.lower().endswith('.root'): ##Here no use of using lower() which returns the lower case of any string, same result even if not using it
            rootFile = rt.TFile(f)
    ...
    if options.inputFitFile is not None:
        ...
        frIn.Print("V")
        for p in rootTools.RootIterator.RootIterator(frIn.floatParsFinal()): ##Return parameters after fit
...
#################################################################
    # get trigger dataset    
    triggerData = None
    # Use uncorrected mgj / pT of wide jets
    corr = '_noCorr'
    if options.triggerDataFile is not None:
        triggerFile = rt.TFile.Open(options.triggerDataFile)
        names = [k.GetName() for k in triggerFile.GetListOfKeys()] ##TFile contains a TKey object for each element in TFile. The key structure contains all the information to uniquely identify a persistent object in a file.
        if 'triggerData' not in names:
            tree = triggerFile.Get("rootTupleTree/tree")      
            triggerData = rt.RooDataSet("triggerData","triggerData",rt.RooArgSet(w.var("mgj"),w.cat("cut")))
            # l1 efficiency:
            if options.l1Trigger:
                cutString = 'abs(deltaETAjj)<1.3&&abs(etaWJ_j1)<2.5&&abs(etaWJ_j2)<2.5&&pTWJ_j1%s>60&&pTWJ_j2%s>30&&PassJSON&&passHLT_CaloJet40_CaloScouting_PFScouting&&mgj>=%i&&mgj<%i'%(corr,corr,w.var("mgj").getMin("Eff"),w.var("mgj").getMax("Eff"))                
            # hlt efficiency:
            else:
                #cutString = 'abs(deltaETAjj)<1.3&&abs(etaWJ_j1)<2.5&&abs(etaWJ_j2)<2.5&&pTWJ_j1%s>60&&pTWJ_j2%s>30&&PassJSON&&passHLT_L1HTT_CaloScouting_PFScouting&&mgj>=%i&&mgj<%i'%(corr,corr,w.var("mgj").getMin("Eff"),w.var("mgj").getMax("Eff"))
                cutString = 'abs(deltaETAjj)<1.3&&abs(etaWJ_j1)<2.5&&abs(etaWJ_j2)<2.5&&pTWJ_j1%s>60&&pTWJ_j2%s>30&&PassJSON&&passHLT_CaloJet40_CaloScouting_PFScouting&&mgj>=%i&&mgj<%i'%(corr,corr,w.var("mgj").getMin("Eff"),w.var("mgj").getMax("Eff"))
            ...
            while True:
                ...
                if entry == -1: break
                tree.GetEntry(entry)          
                a = rt.RooArgSet(w.var('mgj'),w.cat('cut'))   
                a.setRealValue('mgj',eval('tree.mgj%s'%corr))
                # l1 efficiency:
                if options.l1Trigger:
        ...
        w.data('triggerData').fillHistogram(h_numerator,rt.RooArgList(w.var('mgj')),'cut==1')
        w.data('triggerData').fillHistogram(h_denominator,rt.RooArgList(w.var('mgj')))
...
###############################################################    
    ...
    myTH1.Rebin(len(x)-1,'data_obs_rebin',x) ##create a new histogram data_obs_rebin that is clone of myTH1 but with variable binning x.
...
########### trigger fit here #####################
            if options.triggerDataFile is not None and options.doTriggerFit:
                effFr = effFit(w.pdf('effPdf'),w.data('triggerData'),rt.RooArgSet(w.var('mgj')))           
...
############### trigger fit up to here #################
            ...
            fr = simFit(extDijetPdf,dataHist,sideband,w.pdf('effPdf'),w.data('triggerData'),rt.RooArgSet(w.var('mgj')))
            rootTools.Utils.importToWS(w,fr)
            fr.Print('v')    
            fr.covarianceMatrix().Print('v')
            fr.correlationMatrix().Print('v')
    ...
    for model, mass, xsec, signalFileName in zip(models,masses,xsecs,signalFileNames):
        ...
        names = [k.GetName() for k in signalFile.GetListOfKeys()] ##iterate over all the objects in TFile using TKey ##TKey::GetName() will return name of all objects (say histograms) in TFile.
        for name in names:
            d = signalFile.Get(name)
            if isinstance(d, rt.TH1):
                if name=='h_%s_%i'%(model,float(mass)):
                    print name
                    d.Scale(float(xsec)*lumi/d.Integral()) ##d.Integral() gives total number of events. so it is scaling by luminosity. Here the d histogram is already normalized to unity so Integral() = 1. so scaling by lumi will be same if applied at initial level or after cuts. I can simply use it by applying in the PA code and not scaling here. But applying in PA code does not effect here as here we are using prob distribution. So we have to scale here so that the sigal is also emphasized by the data lumi. This is required as otherwise the signal will not corresponds to data lumi. this is also done in old dijet limit code. 
                    if options.triggerDataFile is not None:
    ...
    for i in range(0,len(x)-1):
        ...
        print "%i <= mgj < %i; prediction: %.2f; data %i"  % (x[i],x[i+1],predYield,dataYield)
...
#####################################################
    if options.triggerDataFile is not None:        
        ...
        binning = rt.RooBinning(w.var('mgj').getMin('Eff'),w.var('mgj').getMax('Eff'))
        xEff = array('d',[w.var('mgj').getMin('Eff')])
        ...
        for iBin in plotForLoop:
            if iBin<w.var('mgj').getMax('Eff'):
                binning.addBoundary(iBin)
                xEff.append(iBin)
        xEff.append(w.var('mgj').getMax('Eff'))
        ...
        w.data('triggerData').fillHistogram(h_numerator_coarse,rt.RooArgList(w.var('mgj')),'cut==1')
        w.data('triggerData').fillHistogram(h_denominator_coarse,rt.RooArgList(w.var('mgj')))
        ...
        histo.Draw()
        #histo.GetXaxis().SetRangeUser(w.var('mgj').getMin('Eff'),w.var('mgj').getMax('Eff'))
        if options.l1Trigger:
            histo.GetXaxis().SetRangeUser(w.var('mgj').getMin('Eff'),x[15])
        else:            
            histo.GetXaxis().SetRangeUser(w.var('mgj').getMin('Eff'),x[7])
        #histo.GetXaxis().SetRangeUser(w.var('mgj').getMin('Eff'),1000)
        histo.SetMinimum(0.)
        ...
        effTF1 = w.function('effFunc').asTF(rt.RooArgList(w.var('mgj')))
        ...
        if options.l1Trigger:
            h_eff_residual_vs_mass.GetXaxis().SetRangeUser(w.var('mgj').getMin('Eff'),x[15])    
        else:
            h_eff_residual_vs_mass.GetXaxis().SetRangeUser(w.var('mgj').getMin('Eff'),x[7])    
        ...
        d.Print(options.outDir+"/eff_mgj_%s_%s.pdf"%(fitRegion.replace(',','_'),box))
        d.Print(options.outDir+"/eff_mgj_%s_%s.C"%(fitRegion.replace(',','_'),box))
        tdirectory.cd()
        d.Write()
#################################################
############### trigger output ends here ########
#################################################
    
    background_pdf = w.pdf('%s_bkg_unbin'%box)
    background= background_pdf.asTF(rt.RooArgList(w.var('mgj')),rt.RooArgList(w.var('p0_%s'%box))) ##taking bkg pdf as a function
    int_b = background.Integral(w.var('mgj').getMin(),w.var('mgj').getMax()) ##taking integral with paras p1,p2,p3 after fitting of bkg in mgj range
    #p0_b = w.var('Ntot_bkg_%s'%box).getVal() / (int_b * lumi)
    p0_b = w.var('Ntot_bkg_%s'%box).getVal() / int_b
 #   p0_b =  
...
#Change 1
        ...
        plotRegions = plotRegion.split(',')
        checkInRegions = [g_data.GetX()[i]>w.var('mgj').getMin(reg) and g_data.GetX()[i]<w.var('mgj').getMax(reg) for reg in plotRegions]
...
#Change 2
    ...
    pad_1.SetPad(0.01,0.36,0.99,0.98)
    pad_1.SetLogy()
    #if 'PF' in box or w.var('mgj').getMax() > 2037:
        #pad_1.SetLogx()
    pad_1.SetRightMargin(0.05)
    pad_1.SetTopMargin(0.05)
    pad_1.SetLeftMargin(0.175)
...
#Change 3
        ...
        myRebinnedDensityTH1.SetBinContent(i, myRebinnedTH1.GetBinContent(i))
        myRebinnedDensityTH1.SetBinError(i, myRebinnedTH1.GetBinError(i))
        
        plotRegions = plotRegion.split(',')
        checkInRegions = [myRebinnedDensityTH1.GetXaxis().GetBinCenter(i)>w.var('mgj').getMin(reg) and myRebinnedDensityTH1.GetXaxis().GetBinCenter(i)<w.var('mgj').getMax(reg) for reg in plotRegions]      
        if not any(checkInRegions):
            myRebinnedDensityTH1.SetBinContent(i,0)
            myRebinnedDensityTH1.SetBinError(i,0)
    myRebinnedDensityTH1.GetXaxis().SetRangeUser(w.var('mgj').getMin(),w.var('mgj').getMax())
    myRebinnedDensityTH1.GetYaxis().SetTitle('d#sigma / dm_{gj} [pb / GeV]')
...
#    myRebinnedDensityTH1.Scale(0)
    myRebinnedDensityTH1.SetLineColor(rt.kBlue)
    myRebinnedDensityTH1.SetMarkerColor(rt.kBlue)
    myRebinnedDensityTH1.SetLineWidth(1)    
    #if 'PF' in box:
...
#Change 4
    ...
    myRebinnedDensityTH1.SetMaximum(2e6)
    #if w.var('mgj').getMax() > 2037:            
    #myRebinnedDensityTH1.SetMaximum(20)
...
#    g_data_clone.Draw("pezsame")
 ...
 #   g_data_clone.Draw("pezsame")
    ...
    if 'Calo' in box:
        ...
        if w.var('mgj').getMax() > 2037:
            #pave_sel.AddText("%.1f < m_{gj} < %.1f TeV"%(w.var('mgj').getMin('Low')/1000.,w.var('mgj').getMax('High')/1000.))
            pave_sel.AddText("m_{gj} > %.1f TeV"%(w.var('mgj').getMin('Low')/1000.))
        else:
            pave_sel.AddText("%i < m_{gj} < %i GeV"%(w.var('mgj').getMin('Low'),w.var('mgj').getMax('High')))
    elif 'PF' in box:
    ...
    if 'PF' in box:
        myRebinnedDensityTH1.GetYaxis().SetTitle('d#sigma / dm_{gj} [pb / TeV]')
    ...
    h_fit_residual_vs_mass.GetXaxis().SetRangeUser(w.var('mgj').getMin(),w.var('mgj').getMax())
    ...
    h_fit_residual_vs_mass.GetXaxis().SetTitle('q* Mass [GeV]')
    ...
    c.Print(options.outDir+"/fit_mgj_%s_%s.pdf"%(fitRegion.replace(',','_'),box))
    c.Print(options.outDir+"/fit_mgj_%s_%s.C"%(fitRegion.replace(',','_'),box))
    tdirectory.cd()
    c.Write()
    

    outFileName = "FitResults_%s.root"%(box)
