...
def runToys(w,options,cfg,seed):
    ...
    for i in range(1,len(x)):
                ...
                if inFitRange:                    
                    ...
                    print "Bin = %f " %(iBinX)
                    print "Obs = %f " %(observed)
                    print "Exp = %f " %(expected)
                    print "err = %f" %(pow(getErrorData(observed,expected),2))
                    print "chi2 = %f " %(( observed - expected ) * ( observed - expected ) / pow(getErrorData(observed,expected),2))
                    print "*************************"
                    if observed>0:
