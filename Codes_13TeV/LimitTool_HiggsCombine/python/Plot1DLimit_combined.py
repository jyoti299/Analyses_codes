...
#    xsecFiles = ['data/Qstar_f0p1_xsec.txt']
    xsecFiles = ['data/Bstar_f1p0_xsec.txt']
...
#    Box = Boxes[0]
    ...
    lineColor = {'Qstar_f1p0':rt.kRed+1,
                 'Qstar_f0p5':rt.kMagenta,
                 'Qstar_f0p1':rt.kGray+2,
                 'Bstar_f1p0':rt.kRed+1,
                 'Bstar_f0p5':rt.kRed+1,
                 'Bstar_f0p1':rt.kRed+1,
                 'None':1
                 }
    ...
    legendLabel = {'Qstar_f1p0':'Excited quark (f=1.0)  ',
                   'Qstar_f0p5':'Excited quark (f=0.5)  ',
                   'Qstar_f0p1':'Excited quark (f=0.1)  ',
                   'Bstar_f1p0':'Excited b-quark (f=1.0)',
                   'Bstar_f0p5':'Excited b-quark (f=0.5)',
                   'Bstar_f0p1':'Excited b-quark (f=0.1)'                 
                   }
    ...
    if options.doSignificance:
        ...
        h_limit.SetTitle(" ;Resonance mass [TeV]; #sigma #times #bf{#it{#Beta}} #times #bf{#it{#Alpha}} #times #bf{#it{#epsilon}} [pb]")
    ...
    if options.doSignificance:
        gr_observedLimit[(Box,model)].SetMarkerStyle(29)
        gr_observedLimit[(Box,model)].SetMarkerSize(1.2)
        gr_observedLimit[(Box,model)].SetLineColor(rt.kRed)
        gr_observedLimit[(Box,model)].SetMarkerColor(rt.kBlue+1)
    ...
    if options.model=="Qstar":
        ...
        if len(Boxes)>1:
            l.DrawLatex(0.89,0.75,"b* #rightarrow b#gamma")
        else:
            l.DrawLatex(0.89,0.75,"b* #rightarrow b#gamma")
    ...
    if options.doSignificance:
        ...
        leg = rt.TLegend(0.23,0.19,0.54,0.45)
    ...
    if len(models)==1:
        if options.doSignificance:
            leg.AddEntry(gr_observedLimit[(Box,model)], "Observed Significance","lp")
        else:
    ...
    if options.doSignificance:
        c.SaveAs(options.outDir+"/signif_"+options.model+"_"+options.SigCoup+"_"+options.box.lower()+".pdf")
        c.SaveAs(options.outDir+"/signif_"+options.model+"_"+options.SigCoup+"_"+options.box.lower()+".C")
    else:
        if options.bayes:
            if options.noSys:
