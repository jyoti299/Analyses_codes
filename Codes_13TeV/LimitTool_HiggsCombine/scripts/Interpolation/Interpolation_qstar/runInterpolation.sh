...
Model=Qstar ##Bstar   ##TO_CHANGE

InPath=${pwd}/MassXDist
InSigDir=Qstar_80X_ResShapes_latestSyst_full2016lumi/ ##TO_CHANGE

#for coupling in f1p0 f0p5 f0p1
for coupling in f-0p1 # f-0p1
do

for syst in  nominal JESUP JESDOWN JER PESUP PESDOWN PER
do
...
if [ "${syst}" = "nominal" ]; then
    OutFileName=ResonanceShapes_${Model}_${coupling}_13TeV_PhLID-JetTID-Pt190-nodeta-nodphi-CSVM_mass695_80X_Spring16
    mass_X=h_mass_X_bin1
elif [ "${syst}" = "JESUP" ]; then
    OutFileName=ResonanceShapes_${Model}_${coupling}_13TeV_PhLID-JetTID-Pt190-nodeta-nodphi-CSVM_mass695_80X_Spring16_${syst}
