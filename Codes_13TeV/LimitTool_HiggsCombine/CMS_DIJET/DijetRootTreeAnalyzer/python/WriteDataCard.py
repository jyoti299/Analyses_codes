...
def initializeWorkspace(w,cfg,box,scaleFactor=1.,penalty=False,multi=False,x=None,emptyHist1D=None):
    ...
    if x is None:
        x = array('d', cfg.getBinning(box)[0]) # mjj binning ##as getBinning() in framework/Config.py runs over both mjj and th1x and [0] is for mjj
    
    nBins = len(x)-1
    maxBins = nBins
    
    variables = cfg.getVariablesRange(box, "variables",w) ##This by calling getVariablesRange() defines the variables from box into the workspace along with range to this variable. So it defines var mjj and th1x with ranges into the workspace.
    ...
    for command in commands:
        ...
        if lower.find('sum::')!=-1 or lower.find('prod::')!=-1 or lower.find('expr::')!=-1 or lower.find('roogaussian::')!=-1 or lower.find('rooefficiency:')!=-1:
            ...
            pdf = getattr(rt,myclass)(*args) ###rt.myclass(*args) ==> rt.RooDijetBinPdg(*args) => making RooDijetbinPdf and giving the name pdf.
