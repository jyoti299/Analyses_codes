...
for i in Run2015D_PromptReco_v3  Run2015D_PromptReco_v4
do 
...
PostAna_Dir_final=${PostAna_Dir}/Qstar13TeV_HighPtPhID_Advance
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(HasPrimaryVtx){
	  if(PC > -1){
	    ...
	    h_PFChIsoVsPt[0]                 ->Fill((*phoEt)[PC], (*phoPFChIso)[PC]);
      	    h_PFChIsoVsRho[0]                ->Fill(rho, (*phoPFChIso)[PC]);
      	    h_prof_PFChIsoVsPt[0]            ->Fill((*phoEt)[PC], (*phoPFChIso)[PC]);
	    h_prof_PFChIsoVsRho[0]           ->Fill(rho, (*phoPFChIso)[PC]);
	    
	    h_PFNeuIsoVsPt[0]                ->Fill((*phoEt)[PC], (*phoPFNeuIso)[PC]);
	    h_PFNeuIsoVsRho[0]               ->Fill(rho, (*phoPFNeuIso)[PC]);
	    h_prof_PFNeuIsoVsPt[0]           ->Fill((*phoEt)[PC], (*phoPFNeuIso)[PC]);
	    h_prof_PFNeuIsoVsRho[0]          ->Fill(rho, (*phoPFNeuIso)[PC]);

	    h_PFPhIsoVsPt[0][0]              ->Fill((*phoEt)[PC], (*phoPFPhoIso)[PC]);
	    h_PFPhIsoVsRho[0][0]             ->Fill(rho, (*phoPFPhoIso)[PC]);
	    h_prof_PFPhIsoVsPt[0][0]         ->Fill((*phoEt)[PC], (*phoPFPhoIso)[PC]);
      	    h_prof_PFPhIsoVsRho[0][0]        ->Fill(rho, (*phoPFPhoIso)[PC]);
	    ...
	    h_PFPhIsoVsPt[0][1]       ->Fill((*phoEt)[PC], (Alpha + (*phoPFPhoIso)[PC] - rho*Area - Kappa*(*phoEt)[PC]));
	    h_PFPhIsoVsRho[0][1]      ->Fill(rho, (Alpha + (*phoPFPhoIso)[PC] - rho*Area - Kappa*(*phoEt)[PC]));
            h_prof_PFPhIsoVsPt[0][1]  ->Fill((*phoEt)[PC], (Alpha + (*phoPFPhoIso)[PC] - rho*Area - Kappa*(*phoEt)[PC]));
     	    h_prof_PFPhIsoVsRho[0][1] ->Fill(rho, (Alpha + (*phoPFPhoIso)[PC] - rho*Area - Kappa*(*phoEt)[PC]));
...
public :
   ...
   //Histograms
   TH2F *h_PFChIsoVsPt[1];
   TH2F *h_PFChIsoVsRho[1];
   TProfile *h_prof_PFChIsoVsPt[1];
   TProfile *h_prof_PFChIsoVsRho[1];

   TH2F *h_PFNeuIsoVsPt[1];
   TH2F *h_PFNeuIsoVsRho[1];
   TProfile *h_prof_PFNeuIsoVsPt[1];
   TProfile *h_prof_PFNeuIsoVsRho[1];

   TH2F *h_PFPhIsoVsPt[1][2];
   TH2F *h_PFPhIsoVsRho[1][2];
   TProfile *h_prof_PFPhIsoVsPt[1][2];
   TProfile *h_prof_PFPhIsoVsRho[1][2];
   ...
   virtual Bool_t   PassHLT(ULong64_t trigBit);
   ...
   virtual Int_t    PassHighPtPhotonID(); 
...
/*
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/NTuples/Data/Run2015D_PromptReco_v3/SinglePhoton/crab_job_single_photon_13TeV_GJ2p11Invfb_v3/151127_142519/0000/ggNtuples_25ns_data_335.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/NTuples/Data/Run2015D_PromptReco_v3/SinglePhoton/crab_job_single_photon_13TeV_GJ2p11Invfb_v3/151127_142519/0000/ggNtuples_25ns_data_330.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/NTuples/Data/Run2015D_PromptReco_v3/SinglePhoton/crab_job_single_photon_13TeV_GJ2p11Invfb_v3/151127_142519/0000/ggNtuples_25ns_data_331.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/NTuples/Data/Run2015D_PromptReco_v3/SinglePhoton/crab_job_single_photon_13TeV_GJ2p11Invfb_v3/151127_142519/0000/ggNtuples_25ns_data_332.root/ggNtuplizer/EventTree");
      chain->Add("/eos/uscms/store/user/lpcqstar/13TeV/NTuples/Data/Run2015D_PromptReco_v3/SinglePhoton/crab_job_single_photon_13TeV_GJ2p11Invfb_v3/151127_142519/0000/ggNtuples_25ns_data_333.root/ggNtuplizer/EventTree");
*/
...
Bool_t PostAnalyzer_Data::PassHLT(ULong64_t trigBit){

  Bool_t passedHLT = false;

  if((HLTPho>>trigBit) & 1) passedHLT = true; //HLTPho is a 64 bit integer having only those bits as one whose corresponding triggers have been fired
                                              ...
                                              //shift this bit to the first place and its bitwise AND (&) with 1 will be 1, otherwise not.
  return passedHLT;
}
...
Int_t PostAnalyzer_Data::GoodPhoton(TString phoWP){
...
Bool_t PostAnalyzer_Data::PhotonIDNoIso(Int_t ipho, TString phoWP){
...
Int_t PostAnalyzer_Data::PassPhotonIDNoIso(TString phoWP){
...
Int_t PostAnalyzer_Data::PassHighPtPhotonID(){
...
Int_t PostAnalyzer_Data::GoodJet(Int_t pc){
...
void PostAnalyzer_Data::BookHistograms(){
  file->cd();
    ...
    std::string cut0[1] = {"HighPt"};
    std::string cut[2] = {"NoCorrPhIso", "AllCorrPhIso"};
    for( Int_t hist0 = 0; hist0 < 1; ++hist0 ){
      ...
      sprintf(name, "h_PFNeuIsoVsPt_%s",cut0[hist0].c_str());
      h_PFNeuIsoVsPt[hist0] = new TH2F(name, "PFNeuIsoVsPt", 150, 0.0, 6000.0, 100, 0, 300);
      h_PFNeuIsoVsPt[hist0]->GetYaxis()->SetTitle("PFNeuIso");     h_PFNeuIsoVsPt[hist0]->GetYaxis()->CenterTitle();
      h_PFNeuIsoVsPt[hist0]->GetXaxis()->SetTitle("P_{T}^{#gamma} (GeV)");    h_PFNeuIsoVsPt[hist0]->GetXaxis()->CenterTitle();
      h_PFNeuIsoVsPt[hist0]->Sumw2();

      sprintf(name, "h_PFNeuIsoVsRho_%s",cut0[hist0].c_str());
      h_PFNeuIsoVsRho[hist0] = new TH2F(name, "PFNeuIsoVsRho", 500, 0.0, 50.0, 100, 0, 300);
      h_PFNeuIsoVsRho[hist0]->GetYaxis()->SetTitle("PFNeuIso");     h_PFNeuIsoVsRho[hist0]->GetYaxis()->CenterTitle();
      h_PFNeuIsoVsRho[hist0]->GetXaxis()->SetTitle("Rho");    h_PFNeuIsoVsRho[hist0]->GetXaxis()->CenterTitle();
      h_PFNeuIsoVsRho[hist0]->Sumw2();

      sprintf(name, "h_prof_PFNeuIsoVsPt_%s",cut0[hist0].c_str());
      h_prof_PFNeuIsoVsPt[hist0] = new TProfile(name, "PFNeuIsoVsPt", 150, 0.0, 6000.0, 0, 300);
      h_prof_PFNeuIsoVsPt[hist0]->GetYaxis()->SetTitle("PFNeuIso");     h_prof_PFNeuIsoVsPt[hist0]->GetYaxis()->CenterTitle();
      h_prof_PFNeuIsoVsPt[hist0]->GetXaxis()->SetTitle("P_{T}^{#gamma} (GeV)");   h_prof_PFNeuIsoVsPt[hist0]->GetXaxis()->CenterTitle();
      h_prof_PFNeuIsoVsPt[hist0]->Sumw2();

      sprintf(name, "h_prof_PFNeuIsoVsRho_%s",cut0[hist0].c_str());
      h_prof_PFNeuIsoVsRho[hist0] = new TProfile(name, "PFNeuIsoVsRho", 500, 0.0, 50.0, 0, 300);
      h_prof_PFNeuIsoVsRho[hist0]->GetYaxis()->SetTitle("PFNeuIso");     h_prof_PFNeuIsoVsRho[hist0]->GetYaxis()->CenterTitle();
      h_prof_PFNeuIsoVsRho[hist0]->GetXaxis()->SetTitle("Rho");   h_prof_PFNeuIsoVsRho[hist0]->GetXaxis()->CenterTitle();
      h_prof_PFNeuIsoVsRho[hist0]->Sumw2();
