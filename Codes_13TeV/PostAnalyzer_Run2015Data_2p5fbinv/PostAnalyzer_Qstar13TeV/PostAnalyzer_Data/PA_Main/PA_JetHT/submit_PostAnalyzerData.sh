...
#For Data_JetHT
DatanTuplesDir=root://cmseos.fnal.gov//store/user/lpcqstar/13TeV/NTuples
filenameTag=DATA_JetHT
sourceDir=${DatanTuplesDir}/${filenameTag}/
...
public :
   ...
   // prescaled lower threshold Jet triggers                                                                                                      
   ...
   virtual Bool_t   PassHLT_Jet(vector<ULong64_t> trigBits);
...
// used to generate this class and read the Tree.
   if (tree == 0) {
    ...
    /*  chain->Add("root://cmseos.fnal.gov//store/user/lpcqstar/13TeV/NTuples/DATA/Run2015D_PromptReco_v3/ggNtuples_25ns_data_607.root/ggNtuplizer/EventTree");
...
Bool_t PostAnalyzer_Data::PassHLT_Jet(vector<ULong64_t> trigBits){

  int count = 0;
  bool trigFired = false;

  for(Int_t i = 0; i < trigBits.size(); i++){
    if(HLTJet>>(trigBits[i]) & 1){
      count++;                    
    }                             
  }                               
