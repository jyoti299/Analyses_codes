...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   */
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	h_IsoPhotonIdxVsPt[0]->Fill((*phoEt)[GoodIsoPhotons[ip]], GoodIsoPhotons[ip]+1);
     }
      for(int ii = 0; ii < GoodIsoBarrelPhotons.size(); ii++){
	h_GoodPhotonIdxVsPt[0]->Fill((*phoEt)[GoodIsoBarrelPhotons[ii]], GoodIsoBarrelPhotons[ii]+1);
      }				     
      h_nJets[0]->Fill(GoodIsoJets.size());
      for(int ij = 0; ij < GoodIsoJets.size(); ij++){
	h_JetIdxVsPt[0]->Fill((*jetPt)[GoodIsoJets[ij]], GoodIsoJets[ij]+1);
      }
	  ...
	  if(GoodIsoPhotons.size() > 0){
		...
		if(Pass_JetPt){
		  ...
		  if(Pass_JetEta){
			...
			h_PtPhotJet[0]             ->Fill((*phoEt)[PC], (*jetPt)[JC]);
                        h_etaPhotJet[0]            ->Fill((*phoSCEta)[PC], (*jetEta)[JC]);
			h_cosThetaStar[0]          ->Fill(GetCosThetaStar((*phoSCEta)[PC], (*jetEta)[JC]));

			h_goodPV[1]                ->Fill(GoodVertex);
			h_nIsoPhotons[1]           ->Fill(GoodIsoPhotons.size());  
			h_nGoodPhotons[1]          ->Fill(GoodIsoBarrelPhotons.size()); 
			for(int ip = 0; ip < GoodIsoPhotons.size(); ip++){
			  h_IsoPhotonIdxVsPt[1]->Fill((*phoEt)[GoodIsoPhotons[ip]], GoodIsoPhotons[ip]+1);
			}
			for(int ii = 0; ii < GoodIsoBarrelPhotons.size(); ii++){
			  h_GoodPhotonIdxVsPt[1]->Fill((*phoEt)[GoodIsoBarrelPhotons[ii]], GoodIsoBarrelPhotons[ii]+1);
			}
			h_nJets[1]                 ->Fill(GoodIsoJets.size());
			for(int ij = 0; ij < GoodIsoJets.size(); ij++){
			  h_JetIdxVsPt[1]->Fill((*jetPt)[GoodIsoJets[ij]], GoodIsoJets[ij]+1);
			}
			...
			if(Pass_GJInvtMass && IfMassCut){
			  ...
			  h_PtPhotJet[1]             ->Fill((*phoEt)[PC], (*jetPt)[JC]);
			  h_etaPhotJet[1]            ->Fill((*phoSCEta)[PC], (*jetEta)[JC]);
			  h_cosThetaStar[1]          ->Fill(GetCosThetaStar((*phoSCEta)[PC], (*jetEta)[JC]));

			  h_goodPV[2]                ->Fill(GoodVertex);
			  h_nIsoPhotons[2] ->Fill(GoodIsoPhotons.size());  
			  h_nGoodPhotons[2]-> Fill(GoodIsoBarrelPhotons.size()); 
			  for(int ip = 0; ip < GoodIsoPhotons.size(); ip++){
			    h_IsoPhotonIdxVsPt[2]->Fill((*phoEt)[GoodIsoPhotons[ip]], GoodIsoPhotons[ip]+1);
			  }
			  for(int ii = 0; ii < GoodIsoBarrelPhotons.size(); ii++){
			    h_GoodPhotonIdxVsPt[2]->Fill((*phoEt)[GoodIsoBarrelPhotons[ii]], GoodIsoBarrelPhotons[ii]+1);
			  }
			  h_nJets[2]->Fill(GoodIsoJets.size());
			  for(int ij = 0; ij < GoodIsoJets.size(); ij++){
			    h_JetIdxVsPt[2]->Fill((*jetPt)[GoodIsoJets[ij]], GoodIsoJets[ij]+1);
			  }
