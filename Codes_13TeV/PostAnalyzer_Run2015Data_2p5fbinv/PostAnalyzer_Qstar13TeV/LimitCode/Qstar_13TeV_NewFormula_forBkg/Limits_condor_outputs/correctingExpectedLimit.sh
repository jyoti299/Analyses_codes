...
#Important
#provide input file in $1 

grep "expected bound" $1 > bounds.txt

cut -d',' -f2 bounds.txt | cut -d']' -f1 > array.txt

if [ -f quantiles.cc ]; then
echo "++++++++++++++ Deleting quantiles.cc ++++++++++++++"
rm quantiles.cc
fi

echo "++++++++++++++ Creating quantiles.cc ++++++++++++++"

cat >> quantiles.cc <<EOF
#include <iostream>
#include <fstream>
#include <TMath.h>
#include <vector>
...
##compilation
g++ -Wno-deprecated quantiles.cc -o quantiles.exe -I$ROOTSYS/include -L$ROOTSYS/lib `root-config --cflags` `root-config --libs`
