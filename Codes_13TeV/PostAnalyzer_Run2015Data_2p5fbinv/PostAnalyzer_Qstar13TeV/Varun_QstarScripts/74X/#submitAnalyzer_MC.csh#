...
setenv sourceDir /store/user/lpcqstar/13TeV/NTuples/MC/Signal_Qstar/${i}/
setenv InputFilesPath root://cmsxrootd.fnal.gov/${sourceDir}
...
setenv sourceDir /store/user/lpcqstar/13TeV/NTuples/MC/GJets/${i}/
setenv InputFilesPath root://cmsxrootd.fnal.gov/${sourceDir}
set filesPerJob = 1000 ## Total files : max
endif
##
...
setenv sourceDir /store/user/lpcqstar/13TeV/NTuples/MC/DiJet/${i}/
setenv InputFilesPath root://cmsxrootd.fnal.gov/${sourceDir}
set filesPerJob = 1000 ## Total files : max
endif
##
...
setenv sourceDir /store/user/lpcqstar/13TeV/NTuples/MC/${i}/
setenv InputFilesPath root://cmsxrootd.fnal.gov/${sourceDir}
set filesPerJob = 1000 ## Total files : max
endif
##
...
void PhoJet_Analyzer_MC::Loop()
{
	...
	if(PC > -1 && JC > -1){
	    ...
	    c_InvMass_1p2to2p2  = ( getMass(PC,JC) > 1209.0 && getMass(PC,JC) < 2182.0 );
	}
			...
			if(JC > -1){
				...
				if(c_gjDPhi){
					...
					if(c_InvMass){
						...
						h_jet_NConstituents[2]   ->Fill( (*jetNCH)[JC], EvtWeight);
						h_jet_ChargeMultiplicity[2]->Fill( (*jetNConstituents)[JC], EvtWeight);
...
Bool_t PhoJet_Analyzer_MC::jetID(Int_t ijet, TString jetWP){
    ...
    //    if(jetWP == "tightLepVeto"){ //tightLepVeto  -- MUON-FRACTION-NOT-IN-NTUPLIZER
    //	tightLepVeto = (NHF<0.90 && NEMF<0.90 && NumConst>1 && MUF<0.8) && ((abs(eta)<=2.4 && CHF>0 && CHM>0 && CEMF<0.90) || abs(eta)>2.4) && abs(eta)<=3.0	;
    //	jetID = ( ( (*jetNHF)[ijet] < 0.90 && (*jetNEF)[ijet] < 0.90 &&  (*jetNConstituents)[ijet] > 1 && MUON-FRACTION-NOT-IN-NTUPLIZER < 0.8) &&
    //		  ( (fabs((*jetEta)[ijet]) <= 2.4 &&  (*jetCHF)[ijet] > 0.0 &&  (*jetNCH)[ijet] > 0 && (*jetCEF)[ijet] < 0.99 ) || fabs((*jetEta)[ijet]) > 2.4) && 
    //		  fabs((*jetEta)[ijet]) <= 3.0);
    //    }
...
Bool_t PhoJet_Analyzer_MC::AK8jetID(Int_t ijet, TString jetWP){
    ...
    //    if(jetWP == "tightLepVeto"){ //tightLepVeto  -- MUON-FRACTION-NOT-IN-NTUPLIZER
    //	tightLepVeto = (NHF<0.90 && NEMF<0.90 && NumConst>1 && MUF<0.8) && ((abs(eta)<=2.4 && CHF>0 && CHM>0 && CEMF<0.90) || abs(eta)>2.4) && abs(eta)<=3.0	;
    //	jetID = ( ( (*AK8JetNHF)[ijet] < 0.90 && (*AK8JetNEF)[ijet] < 0.90 &&  (*AK8Jetnconstituents)[ijet] > 1 && MUON-FRACTION-NOT-IN-NTUPLIZER < 0.8) &&
    //		  ( (fabs((*AK8JetEta)[ijet]) <= 2.4 &&  (*AK8JetCHF)[ijet] > 0.0 &&  (*AK8JetNCH)[ijet] > 0 && (*AK8JetCEF)[ijet] < 0.99 ) || fabs((*AK8JetEta)[ijet]) > 2.4) && 
    //		  fabs((*AK8JetEta)[ijet]) <= 3.0);
    //    }
