   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(PC > -1){
	  Double_t mcfm_kfac = MCFM_kFact((*phoEt)[PC]);
          if(mcfm_kfac > 1.0) mcfm_kfac = 1.0; //This one for this varbin only and not for all. As here in one bin, there is very large k-factor
	  //uncomment in script
...
Double_t PostAnalyzer_MC::MCFM_kFact(Double_t phpt){
  ...
  TFile *f = TFile::Open("/uscms_data/d3/rocky86/slc6_amd64_gcc491/Analyzer_13TeV/PostAnalyzer_MC/MCFM_kFactor/MCFM_kFact_lordvsvirt_varAndUnitbin_photonpt_dynamicscale.root");
  TH1F *h = (TH1F*)f->Get("h_MCFMnlo_over_MCFMlo_DiffXS_vs_PhotPt_varbin");
