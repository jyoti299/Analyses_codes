...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   //Lumi = 2502.816; // (pb^{-1}), COMPLETE 2015 25ns DATA WITH SILVER JSON WITH OLD REFERENCE FILE
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	Total_EvtWt = PreBTag_EvtWt;
      ...
      h_nIsoPhotons[0] ->Fill(GoodIsoPhotons.size(), Total_EvtWt);  // Tot # of isolated photons     
      h_nGoodPhotons[0]-> Fill(GoodIsoBarrelPhotons.size(), Total_EvtWt); // Tot # of isolated photons with pt > cut and eta < cut      
      for(int ip = 0; ip < GoodIsoPhotons.size(); ip++){
        h_IsoPhotonIdxVsPt[0]->Fill((*phoEt)[GoodIsoPhotons[ip]], GoodIsoPhotons[ip]+1, Total_EvtWt);
      }
      for(int ii = 0; ii < GoodIsoBarrelPhotons.size(); ii++){
	h_GoodPhotonIdxVsPt[0]->Fill((*phoEt)[GoodIsoBarrelPhotons[ii]], GoodIsoBarrelPhotons[ii]+1, Total_EvtWt);
      }
      h_nJets[0]->Fill(GoodIsoJets.size(), Total_EvtWt);
      for(int ij = 0; ij < GoodIsoJets.size(); ij++){
        h_JetIdxVsPt[0]->Fill((*jetPt)[GoodIsoJets[ij]], GoodIsoJets[ij]+1, Total_EvtWt);
      }
		...
		if(Pass_JetPt){
		  ...
		  if(Pass_JetEta){
			...
			h_CutFlowWithWts->Fill(9.5, Total_EvtWt);
			  ...
			  h_SumPFMet[0]              ->Fill(pfMETsumEt, Total_EvtWt);
			  h_MetBySumMET[0]          ->Fill(pfMET/pfMETsumEt, Total_EvtWt);
			  ...
			  h_PtPhotJet[0]             ->Fill((*phoEt)[PC], (*jetPt)[JC], Total_EvtWt);
			  h_etaPhotJet[0]            ->Fill((*phoSCEta)[PC], (*jetEta)[JC], Total_EvtWt);
			  h_cosThetaStar[0]          ->Fill(GetCosThetaStar((*phoSCEta)[PC], (*jetEta)[JC]), Total_EvtWt);

			  h_goodPV_TotalWt[1]        ->Fill(GoodVertex, Total_EvtWt);
			  h_nIsoPhotons[1]->Fill(GoodIsoPhotons.size(), Total_EvtWt);                                                    
			  h_nGoodPhotons[1]-> Fill(GoodIsoBarrelPhotons.size(), Total_EvtWt);                     
			  for(int ip = 0; ip < GoodIsoPhotons.size(); ip++){
			    h_IsoPhotonIdxVsPt[1]->Fill((*phoEt)[GoodIsoPhotons[ip]], GoodIsoPhotons[ip]+1, Total_EvtWt);
			  }
			  for(int ii = 0; ii < GoodIsoBarrelPhotons.size(); ii++){
			    h_GoodPhotonIdxVsPt[1]->Fill((*phoEt)[GoodIsoBarrelPhotons[ii]], GoodIsoBarrelPhotons[ii]+1, Total_EvtWt);
			  }
			  h_nJets[1]->Fill(GoodIsoJets.size(), Total_EvtWt);
			  for(int ij = 0; ij < GoodIsoJets.size(); ij++){
			    h_JetIdxVsPt[1]->Fill((*jetPt)[GoodIsoJets[ij]], GoodIsoJets[ij]+1, Total_EvtWt);
			  }
			  ...
			  if(Pass_GJInvtMass && IfMassCut){
			    ...
			    h_SumPFMet[1]              ->Fill(pfMETsumEt, Total_EvtWt);
			    h_MetBySumMET[1]          ->Fill(pfMET/pfMETsumEt, Total_EvtWt);
			    ...
			    h_PtPhotJet[1]             ->Fill((*phoEt)[PC], (*jetPt)[JC], Total_EvtWt);
			    h_etaPhotJet[1]            ->Fill((*phoSCEta)[PC], (*jetEta)[JC], Total_EvtWt);
			    h_cosThetaStar[1]          ->Fill(GetCosThetaStar((*phoSCEta)[PC], (*jetEta)[JC]), Total_EvtWt);

			    h_goodPV_TotalWt[2]        ->Fill(GoodVertex, Total_EvtWt);
			    h_nIsoPhotons[2] ->Fill(GoodIsoPhotons.size(), Total_EvtWt);  
			    h_nGoodPhotons[2]-> Fill(GoodIsoBarrelPhotons.size(), Total_EvtWt);
			    for(int ip = 0; ip < GoodIsoPhotons.size(); ip++){
			      h_IsoPhotonIdxVsPt[2]->Fill((*phoEt)[GoodIsoPhotons[ip]], GoodIsoPhotons[ip]+1, Total_EvtWt);
			    }
			    for(int ii = 0; ii < GoodIsoBarrelPhotons.size(); ii++){
			      h_GoodPhotonIdxVsPt[2]->Fill((*phoEt)[GoodIsoBarrelPhotons[ii]], GoodIsoBarrelPhotons[ii]+1, Total_EvtWt);
			    }
			    h_nJets[2]->Fill(GoodIsoJets.size(), Total_EvtWt);
			    for(int ij = 0; ij < GoodIsoJets.size(); ij++){
			      h_JetIdxVsPt[2]->Fill((*jetPt)[GoodIsoJets[ij]], GoodIsoJets[ij]+1, Total_EvtWt);
			    }
