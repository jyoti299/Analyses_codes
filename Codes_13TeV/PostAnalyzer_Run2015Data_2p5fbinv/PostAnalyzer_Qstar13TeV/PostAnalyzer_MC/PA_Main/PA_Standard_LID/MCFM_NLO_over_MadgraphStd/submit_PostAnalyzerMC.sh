...
do 
#XS_LO=( 20790.0   9238.0  2305.0  274.4  93.46 )
XS=( 20790.0   9238.0   2305.0   274.4   93.46  )
...
//   TString OutputPath = "${destinationDir}/";
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(PC > -1){
	  ...
	  //uncomment in script
	  Double_t mult = (${XS[${sampleIndex}]});
	  //comment out in script
	  //Double_t mult = 1;
	  Double_t fac = mcfm_kfac/mult;
	  Total_EvtWt = PreBTag_EvtWt * fac;
	  if(mcfm_kfac == 0){
...
Double_t PostAnalyzer_MC::MCFM_kFact(Double_t phpt){
  ...
  TFile *f = TFile::Open("/uscms_data/d3/rocky86/slc6_amd64_gcc491/Analyzer_13TeV/PostAnalyzer_MC/MCFM_kFactor/dirgam_tota_CT10.00_91__91__GJet_NLO.root");
  TH1F *h = (TH1F*)f->Get("id1");
  Int_t bin = h->GetXaxis()->FindBin(phpt);
  Double_t bincont = h->GetBinContent( bin );
  return bincont * 1000; // to make XS from fb to pb.
