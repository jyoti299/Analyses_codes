...
//      PU_EvtWt = PUWeights((*puTrue)[0]);
	...
	for(Int_t ip = 0; ip <  HighPtPhotons.size(); ip++){
      ...
      h_nPhotons[0]        ->Fill(HighPtPhotons.size(), PreBTag_EvtWt);
		...
		if(JC > -1){
		  ...
		  if(Pass_JetPt){
			...
			if(Pass_GJdEta){
			  ...
			  h_nPhotons[1]              ->Fill(HighPtPhotons.size(), PreBTag_EvtWt);
			  ...
			  if(Pass_GJInvtMass && IfMassCut){
			    ...
			    h_nPhotons[2]              ->Fill(HighPtPhotons.size(), PreBTag_EvtWt);
