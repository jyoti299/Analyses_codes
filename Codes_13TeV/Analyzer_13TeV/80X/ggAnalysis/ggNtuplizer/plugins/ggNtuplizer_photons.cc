...
vector<float>  phoCalibE_WithSlewRateSol_;
vector<float>  phoCalibEt_WithSlewRateSol_;
...
vector<double>   phoScaleCorrUnc_; //defined by me
vector<double>   phoSmearUnc_nominal_; //defined by me
vector<double>   phoSmearUnc_rhoUp_; //defined by me
vector<double>   phoSmearUnc_rhoDown_; //defined by me
vector<double>   phoSmearUnc_phiUp_; //defined by me
vector<double>   phoSmearUnc_phiDown_; //defined by me
...
void ggNtuplizer::branchesPhotons(TTree* tree) {
  ...
  tree->Branch("phoCalibE_WithSlewRateSol",               &phoCalibE_WithSlewRateSol_);
  tree->Branch("phoCalibEt_WithSlewRateSol",              &phoCalibEt_WithSlewRateSol_);
  ...
  tree->Branch("phomaxXtalenergyFull5x5", &phomaxXtalenergyFull5x5_);
  tree->Branch("phoseedTimeFull5x5",      &phoseedTimeFull5x5_);
  tree->Branch("phoIEta",                 &phoIEta_); 
  tree->Branch("phoIPhi",                 &phoIPhi_); 
  ...
  if(doSyst_){
    tree->Branch("phoScaleCorrUnc",                  &phoScaleCorrUnc_);
    tree->Branch("phoSmearUnc_nominal",              &phoSmearUnc_nominal_);
    tree->Branch("phoSmearUnc_rhoUp",                &phoSmearUnc_rhoUp_);
    tree->Branch("phoSmearUnc_rhoDown",              &phoSmearUnc_rhoDown_);
    tree->Branch("phoSmearUnc_phiUp",                &phoSmearUnc_phiUp_);
    tree->Branch("phoSmearUnc_phiDown",              &phoSmearUnc_phiDown_);
  }
...
void ggNtuplizer::fillPhotons(const edm::Event& e, const edm::EventSetup& es) {
  ...
  phoCalibE_            .clear();
  phoCalibEt_           .clear();
  phoCalibE_WithSlewRateSol_.clear();
  phoCalibEt_WithSlewRateSol_.clear();
  phoSCE_               .clear();
  phoSCRawE_            .clear();
  phoESEn_              .clear();
  phoESEnP1_            .clear();
  ...
  phomaxXtalenergyFull5x5_.clear();
  ...
  */
  if(doSyst_){
    phoScaleCorrUnc_         .clear();
    phoSmearUnc_nominal_     .clear();
    phoSmearUnc_rhoUp_       .clear();
    phoSmearUnc_rhoDown_     .clear();
    phoSmearUnc_phiUp_       .clear();
    phoSmearUnc_phiDown_     .clear();
  }
  ...
  //Scale correction class for photon scale and smearing systematics
  EnergyScaleCorrection_class eScaler("EgammaAnalysis/ElectronTools/data/ScalesSmearings/Winter_2016_reReco_v1_ele");
  //EnergyScaleCorrection_class eScaler(EleScale_Smearing_file_); //this is not working. Not able to find the file by this way.
  ...
  for (edm::View<pat::Photon>::const_iterator iPho = photonHandle->begin(); iPho != photonHandle->end(); ++iPho) {
    ...
    phoIEta_          .push_back(iPho->iEta());
    phoIPhi_          .push_back(iPho->iPhi());
    ...
    if(theSeedHit != rechits->end()){
      phoseedTimeFull5x5_.push_back((*theSeedHit).time());
    }else{
      phoseedTimeFull5x5_.push_back(100.0);
      //std::cout<<"No rechits found:"<<std::endl;
    }
    
    //----------------------------------------------
    //This code snippet is for solution of slew-rate problem, affecting the pulse shape recostruction for electrons/photons 
    //encountering the gain switch turning point in barrel region. This is yet not added into EGMSmerear and hence applied on top on that here.
    //From the next versions, it will be merged into EGMSmerear so check it before applying it next time.
    //The recommendations for this are found in the twiki:https://twiki.cern.ch/twiki/bin/view/CMS/EGMSmearer   
    const EcalRecHit * rh = NULL;
    double Ecorr=1;
    if(isBarrel){
      auto rh_i = rechits->find(seed);
      if( rh_i != rechits->end()) rh = &(*rh_i);
      else rh = NULL;
    }
    if(rh==NULL) Ecorr=1;
    else{
    ...
    phoCalibEt_WithSlewRateSol_.push_back(corrPt*Ecorr);
    phoCalibE_WithSlewRateSol_.push_back(corrEn*Ecorr);
    //Solution of slew rate energy problem ends here
    //----------------------------------------------
    ...
    if(doSyst_){    
      ...
      //Scale
      double error_scale = eScaler.ScaleCorrectionUncertainty(e.id().run(), iPho->isEB(), iPho->r9(), (*iPho).superCluster()->eta(), iPho->et());
      phoScaleCorrUnc_.push_back(error_scale);

      //Smearing
      double sigma_nominal = eScaler.getSmearingSigma(e.id().run(), iPho->isEB(), iPho->r9(), (*iPho).superCluster()->eta(), iPho->et(), 0, 0);
      double sigma_rhoUp = eScaler.getSmearingSigma(e.id().run(), iPho->isEB(), iPho->r9(), (*iPho).superCluster()->eta(), iPho->et(), 1, 0);
      double sigma_rhoDown = eScaler.getSmearingSigma(e.id().run(), iPho->isEB(), iPho->r9(), (*iPho).superCluster()->eta(), iPho->et(), -1, 0);
      double sigma_phiUp = eScaler.getSmearingSigma(e.id().run(), iPho->isEB(), iPho->r9(), (*iPho).superCluster()->eta(), iPho->et(), 0, 1);
      double sigma_phiDown = eScaler.getSmearingSigma(e.id().run(), iPho->isEB(), iPho->r9(), (*iPho).superCluster()->eta(), iPho->et(), 0, -1);

      TRandom *rgen_ = new TRandom3(0);
      double smear_sigma_nominal = rgen_->Gaus(1, sigma_nominal);
      double smear_sigma_rhoUp = rgen_->Gaus(1, sigma_rhoUp);
      double smear_sigma_rhoDown = rgen_->Gaus(1, sigma_rhoDown);
      double smear_sigma_phiUp = rgen_->Gaus(1, sigma_phiUp);
      double smear_sigma_phiDown = rgen_->Gaus(1,sigma_phiDown);

      phoSmearUnc_nominal_.push_back(smear_sigma_nominal);
      phoSmearUnc_rhoUp_.push_back(smear_sigma_rhoUp);
      phoSmearUnc_rhoDown_.push_back(smear_sigma_rhoDown);
      phoSmearUnc_phiUp_.push_back(smear_sigma_phiUp);
      phoSmearUnc_phiDown_.push_back(smear_sigma_phiDown);
    ...
    phomaxXtalenergyFull5x5_ .push_back(lazyToolnoZS.eMax(*((*iPho).superCluster()->seed())));
