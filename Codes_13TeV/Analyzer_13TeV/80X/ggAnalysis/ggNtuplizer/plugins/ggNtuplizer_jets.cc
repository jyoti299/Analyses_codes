...
vector<float> AK8JetPx_; //defined by me
vector<float> AK8JetPy_; //defined by me
vector<float> AK8JetPz_; //defined by me
...
void ggNtuplizer::branchesJets(TTree* tree) {
  ...
  tree->Branch("jetJERSmearingDown",    &jetJERSmearingDown_);
  ...
  if (dumpSubJets_) {
    ...
    tree->Branch("AK8JetPx",                 &AK8JetPx_);
    tree->Branch("AK8JetPy",                 &AK8JetPy_);
    tree->Branch("AK8JetPz",                 &AK8JetPz_);
...
void ggNtuplizer::fillJets(const edm::Event& e, const edm::EventSetup& es) {
  ...
  e.getByToken(ak4GenJetLabel_, genJetsHandle);

  edm::Handle<double> rhoHandleForJER;
  e.getByToken(rhoLabelForJER_, rhoHandleForJER);
  ...
  resolution = JME::JetResolution::get(es, "AK4PFchs_pt");
  res_sf = JME::JetResolutionScaleFactor::get(es, "AK4PFchs");
  ...
  for (edm::View<pat::Jet>::const_iterator iJet = jetHandle->begin(); iJet != jetHandle->end(); ++iJet) {
    ...
    JME::JetParameters resParam;
    resParam.setJetPt(iJet->pt());
    resParam.setJetEta(iJet->eta());
    resParam.setRho(*rhoHandleForJER);
    float res = resolution.getResolution(resParam);
    float abs_res = iJet->pt()*res;

    JME::JetParameters sfParam = {{JME::Binning::JetEta, iJet->eta()}, {JME::Binning::Rho, *rhoHandleForJER}};
    float sf = res_sf.getScaleFactor(sfParam);
    float sf_up = res_sf.getScaleFactor(sfParam, Variation::UP);
    float sf_down = res_sf.getScaleFactor(sfParam, Variation::DOWN);

    Double_t min_dR = 0.2; //Rcone/2
    const reco::GenJet* matched_genJet = nullptr;
    if(genJetsHandle.isValid()){
      for (vector<reco::GenJet>::const_iterator ij = genJetsHandle->begin(); ij != genJetsHandle->end(); ++ij) {
	const reco::GenJet *j = &(*ij);
	Double_t dR = deltaR(iJet->eta(), iJet->phi(), j->eta(), j->phi());
	if(dR < min_dR){
	  Double_t dpt = fabs(iJet->pt() - j->pt());
	  if(dpt < 3*abs_res){
	    matched_genJet = j;
	    break;
	  }
    ...
    Double_t newPt, newPt_up, newPt_down;
    if(matched_genJet){
      newPt = TMath::Max(0.0, (matched_genJet->pt()+sf*(iJet->pt() - matched_genJet->pt())));
      newPt_up = TMath::Max(0.0, (matched_genJet->pt()+sf_up*(iJet->pt() - matched_genJet->pt())));
      newPt_down = TMath::Max(0.0, (matched_genJet->pt()+sf_down*(iJet->pt() - matched_genJet->pt())));
    }else{
      if(sf > 1){
	Double_t sigma = sqrt(sf*sf -1)*res;
	TRandom *rgen_ = new TRandom3(0);
	Double_t smear = rgen_->Gaus(1, sigma);
	newPt = iJet->pt()+smear;
      }else{
	newPt = iJet->pt();
      }
      if(sf_up > 1){
	Double_t sigma_up = sqrt(sf_up*sf_up -1)*res;
	TRandom *rgen_ = new TRandom3(0);
	Double_t smear_up = rgen_->Gaus(1, sigma_up);
	newPt_up = iJet->pt()+smear_up;
      }else{
	newPt_up = iJet->pt();
      }
      if(sf_down > 1){
	Double_t sigma_down = sqrt(sf_down*sf_down -1)*res;
	TRandom *rgen_ = new TRandom3(0);
	Double_t smear_down = rgen_->Gaus(1, sigma_down);
	newPt_down = iJet->pt()+smear_down;
      }else{
	newPt_down = iJet->pt();
      }
    }  
    
    Double_t smear_factor = newPt/iJet->pt();
    Double_t smear_factor_up = newPt_up/iJet->pt();
    Double_t smear_factor_down = newPt_down/iJet->pt();
    jetJERSmearing_.push_back(smear_factor);
    jetJERSmearingUp_.push_back(smear_factor_up);
    jetJERSmearingDown_.push_back(smear_factor_down);
    //--------------------
...
//    e.getByToken(boostedDoubleSVLabel_, pfBoostedDoubleSecondaryVertex); 
    ...
    for(ijetAK8 = beginAK8; ijetAK8 != endAK8; ++ijetAK8 ) {
      ...
      AK8JetPx_.push_back(    ijetAK8->px());
      AK8JetPy_.push_back(    ijetAK8->py());
      AK8JetPz_.push_back(    ijetAK8->pz());
...
//This part is having issue when running on DYJets100To250 dataset::: so false values in these branches for EWK:DYJets100To250, Qstar_f0p1, Bstarf0p1
      AK8puppiPt_.push_back(-99);// ijetAK8->userFloat("ak8PFJetsPuppiValueMap:pt"));
      AK8puppiMass_.push_back(-99);// ijetAK8->userFloat("ak8PFJetsPuppiValueMap:mass"));
      AK8puppiEta_.push_back(-99);// ijetAK8->userFloat("ak8PFJetsPuppiValueMap:eta"));
      AK8puppiPhi_.push_back(-99);// ijetAK8->userFloat("ak8PFJetsPuppiValueMap:phi"));
      AK8puppiTau1_.push_back(-99);// ijetAK8->userFloat("ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau1"));
      AK8puppiTau2_.push_back(-99);// ijetAK8->userFloat("ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau2"));
      AK8puppiTau3_.push_back(-99);// ijetAK8->userFloat("ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau3"));
      ...
      //nPuppiSJs++;
      vecPuppiSDSJpt.push_back(-99);//puppiSDSJ->pt());
      vecPuppiSDSJeta.push_back(-99);//puppiSDSJ->eta());
      vecPuppiSDSJmass.push_back(-99);//puppiSDSJ->mass());
      vecPuppiSDSJphi.push_back(-99);//puppiSDSJ->phi());
      vecPuppiSDSJe.push_back(-99);//puppiSDSJ->energy());
      vecPuppiSDSJflavour.push_back(-99);//abs(puppiSDSJ->partonFlavour()));
      vecPuppiSDSJcharge.push_back(-99);//puppiSDSJ->charge());
      vecPuppiSDSJcsv.push_back(-99);//puppiSDSJ->bDiscriminator("pfCombinedInclusiveSecondaryVertexV2BJetTags") );
...
//fir L2L3 corrections
      ...
      AK8puppiSDL2L3corr_.push_back(-99);//corr_puppiSD);
      AK8puppiSDMass_.push_back(-99);//puppi_softdrop.M());
      AK8puppiSDMassL2L3Corr_.push_back(-99);//corr_puppiSD*puppi_softdrop.M());
