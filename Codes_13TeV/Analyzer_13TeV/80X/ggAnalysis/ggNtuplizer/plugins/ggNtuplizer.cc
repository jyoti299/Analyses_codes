...
ggNtuplizer::ggNtuplizer(const edm::ParameterSet& ps) {
  ...
  doSyst_                    = ps.getParameter<bool>("doSyst");
  ...
  EleScale_Smearing_file_    = ps.getParameter<std::string>("EleScale_Smearing_file");
  ...
  rhoLabelForJER_            = consumes<double>                        (ps.getParameter<InputTag>("rhoLabelForJER")); 
  ...
  ak4GenJetLabel_            = consumes<vector<reco::GenJet> >         (ps.getParameter<InputTag>("ak4GenJetSrc"));
