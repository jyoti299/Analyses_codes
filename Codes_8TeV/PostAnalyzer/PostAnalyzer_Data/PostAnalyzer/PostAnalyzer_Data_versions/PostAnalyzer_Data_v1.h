...
void PostAnalyzer_Data::BookHistograms(){
  ...
  h_BJetPt = new TH1F("h_BJetPt", "Pt Distribution of BJets", 100, 20.0, 2520.0);
  h_BJetPt->GetYaxis()->SetTitle("Events/25 GeV");            h_BJetPt->GetYaxis()->CenterTitle();
  h_BJetPt->GetXaxis()->SetTitle("P_{T}^{BJet}");             h_BJetPt->GetXaxis()->CenterTitle();
  h_BJetPt->Sumw2();

  h_BJetEta = new TH1F("h_BJetEta", "Eta Distribution of BJets", 200, -5.0, 5.0);
  h_BJetEta->GetYaxis()->SetTitle("Events");                 h_BJetEta->GetYaxis()->CenterTitle();
  h_BJetEta->GetXaxis()->SetTitle("{#eta}^{BJet}");          h_BJetEta->GetXaxis()->CenterTitle();
  h_BJetEta->Sumw2();

  h_BJetPhi = new TH1F("h_BJetPhi", "Phi Distribution of BJets", 200, -5.0, 5.0);
  h_BJetPhi->GetYaxis()->SetTitle("Events");                 h_BJetPhi->GetYaxis()->CenterTitle();
  h_BJetPhi->GetXaxis()->SetTitle("{#phi}^{BJet}");          h_BJetPhi->GetXaxis()->CenterTitle();
  h_BJetPhi->Sumw2();
  ...
  h_GBJetInvtMass = new TH1F("h_GBJetInvtMass", "Invarient Mass Distribution of Gamma+BJet", 160, 0.0, 4000.0);
  h_GBJetInvtMass->GetYaxis()->SetTitle("Events/25 GeV");     h_GBJetInvtMass->GetYaxis()->CenterTitle();
  h_GBJetInvtMass->GetXaxis()->SetTitle("M({#gamma} + BJet)"); h_GBJetInvtMass->GetXaxis()->CenterTitle();
  h_GBJetInvtMass->Sumw2();

  h_GBJetdEta = new TH1F("h_GBJetdEta", "DeltaEta Distribution Between Gamma & BJet", 120, 0.0, 6.0);
  h_GBJetdEta->GetYaxis()->SetTitle("Events");                h_GBJetdEta->GetYaxis()->CenterTitle();
  h_GBJetdEta->GetXaxis()->SetTitle("#Delta #eta");           h_GBJetdEta->GetXaxis()->CenterTitle();
  h_GBJetdEta->Sumw2();

  h_GBJetdPhi = new TH1F("h_GBJetdPhi", "DeltaPhi Distribution Between Gamma & BJet", 64, 0.0, 3.2);
  h_GBJetdPhi->GetYaxis()->SetTitle("Events");                h_GBJetdPhi->GetYaxis()->CenterTitle();
  h_GBJetdPhi->GetXaxis()->SetTitle("#Delta #phi");           h_GBJetdPhi->GetXaxis()->CenterTitle();
  h_GBJetdPhi->Sumw2();

  h_GBJetdR = new TH1F("h_GBJetdR", "DeltaR Distribution Between Gamma & BJet", 100, 0.0, 10.0);
  h_GBJetdR->GetYaxis()->SetTitle("Events");                  h_GBJetdR->GetYaxis()->CenterTitle();
  h_GBJetdR->GetXaxis()->SetTitle("#Delta R");                h_GBJetdR->GetXaxis()->CenterTitle();
  h_GBJetdR->Sumw2();

  h_BJetsFrac = new TH1F("h_BJetsFrac", "Fraction of BJets among all Jets", 100, 0.0, 1.0);
  h_BJetsFrac->GetYaxis()->SetTitle("Events");                h_BJetsFrac->GetYaxis()->CenterTitle();
  h_BJetsFrac->GetXaxis()->SetTitle("Fraction of BJets");     h_BJetsFrac->GetXaxis()->CenterTitle();
  h_BJetsFrac->Sumw2();
