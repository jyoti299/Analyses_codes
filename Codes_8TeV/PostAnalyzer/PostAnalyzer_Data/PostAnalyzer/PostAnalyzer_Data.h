...
Double_t PostAnalyzer_Data::GetLICTD(Int_t i){
  ...
  }//This basically consider the crystal with max energy and substitute SeedE = max xtal energy, seeTime = max xtal time and crysIdx = max xtal idx
   // since seed crysal is the one that has the max energy so it basically considers the seed crystal for the photon. 
