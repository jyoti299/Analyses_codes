...
sourceDir=/eos/uscms/store/user/lpcqstar/NTuples/Data/Data22Jan2013ReReco/Data_Run2012D_22Jan_AOD_NTuples/
DataSetName=Data_Run2012D_22Jan_AOD_NTuples

if [ -f Dataset_Run2012D.txt ]; then
echo "++++++++++++++ Deleting Dataset_Run2012D.txt ++++++++++++++"
rm Dataset_Run2012D.txt
fi

ls -ltr --format=single-column ${sourceDir} > Dataset_Run2012D.txt

dataset=Dataset_Run2012D.txt

Total_files=$(wc -l ${dataset} | cut -c1-4)

echo "Total number of files in Dataset D are ${Total_files}"

r=300
echo "Max Files per Job = ${r}"

Total_jobs=$((((${Total_files}/${r}))+1))

echo "Total Jobs to be submitted = ${Total_jobs}"

sf=1        #start file in a job
maxf=${r}   #max files in a job 

while [ ${sf} -le ${Total_files} ]
do

#if [ -f Subset_Run2012D.txt ]; then
#echo "++++++++++++++ Deleting Subset_Run2012D.txt ++++++++++++++"
#rm Subset_Run2012D.txt
#fi

#To get set of 300 files from the full set every time
tail -n +${sf} ${dataset} | head -n ${r} > Subset_Run2012D_${sf}_${maxf}.txt
subdataset=Subset_Run2012D_${sf}_${maxf}.txt

if [ ${maxf} -le ${Total_files} ]
then
filenameTag=${DataSetName}_${sf}_${maxf}
else
filenameTag=${DataSetName}_${sf}_${Total_files}
fi
...
outputDir=${PostAna_Dir_mediumPhId}/Data
...
###***********NO CHANGE IN PostAnalyzer_Data.C (SAME AS FOR RUN2012ABC**********###
		  ...
		  if(JC > -1){
			...
			if(Pass_GJdPhicut){
			  ...
			  if(Pass_GJdEtacut){
				...
				h_CSVL_GBJetInvtMass_UnitBinning->Fill(GetInvtMass(PC, JC));
				...
				h_CSVM_GBJetInvtMass_UnitBinning->Fill(GetInvtMass(PC, JC));
				...
				h_CSVT_GBJetInvtMass_UnitBinning->Fill(GetInvtMass(PC, JC));
...
public :
   ...
   TH1F *h_CSVL_GBJetInvtMass_binning40;
   TH1F *h_CSVL_GBJetInvtMass_VariableBinning;
   TH1F *h_CSVL_GBJetInvtMass_UnitBinning;
   ...
   TH1F *h_CSVM_GBJetInvtMass_binning40;
   TH1F *h_CSVM_GBJetInvtMass_VariableBinning;
   TH1F *h_CSVM_GBJetInvtMass_UnitBinning;
   ...
   TH1F *h_CSVT_GBJetInvtMass_binning40;
   TH1F *h_CSVT_GBJetInvtMass_VariableBinning;
   TH1F *h_CSVT_GBJetInvtMass_UnitBinning;
   ...
   TH1F *h_PassHLT150_PassProbes;
   TH1F *h_PassHLT75OR90_AllProbes;
...
void PostAnalyzer_Data::BookHistograms(){
  ...
  */
  ...
  h_CSVL_GBJetInvtMass_binning40 = new TH1F("h_CSVL_GBJetInvtMass_binning40", "Invarient Mass Distribution of Gamma+CSVL-BJet", 125, 0.0, 5000.0);
  h_CSVL_GBJetInvtMass_binning40->GetYaxis()->SetTitle("Events/40 GeV");     h_CSVL_GBJetInvtMass_binning40->GetYaxis()->CenterTitle();
  h_CSVL_GBJetInvtMass_binning40->GetXaxis()->SetTitle("M({#gamma} + BJet)"); h_CSVL_GBJetInvtMass_binning40->GetXaxis()->CenterTitle();
  h_CSVL_GBJetInvtMass_binning40->Sumw2();

  h_CSVL_GBJetInvtMass_VariableBinning = new TH1F("h_CSVL_GBJetInvtMass_VariableBinning", "Invarient Mass Dist. of Gamma+CSVL-BJet", nbins, xbins);
  h_CSVL_GBJetInvtMass_VariableBinning->GetYaxis()->SetTitle("Events/Variable");     h_CSVL_GBJetInvtMass_VariableBinning->GetYaxis()->CenterTitle();
  h_CSVL_GBJetInvtMass_VariableBinning->GetXaxis()->SetTitle("M({#gamma} + BJet)"); h_CSVL_GBJetInvtMass_VariableBinning->GetXaxis()->CenterTitle();
  h_CSVL_GBJetInvtMass_VariableBinning->Sumw2();

  h_CSVL_GBJetInvtMass_UnitBinning = new TH1F("h_CSVL_GBJetInvtMass_UnitBinning", "Invarient Mass Dist. of Gamma+CSVL-BJet", 14000, 0, 14000);
  h_CSVL_GBJetInvtMass_UnitBinning->GetYaxis()->SetTitle("Events/UnitBin");     h_CSVL_GBJetInvtMass_UnitBinning->GetYaxis()->CenterTitle();
  h_CSVL_GBJetInvtMass_UnitBinning->GetXaxis()->SetTitle("M({#gamma} + BJet)");  h_CSVL_GBJetInvtMass_UnitBinning->GetXaxis()->CenterTitle();
  h_CSVL_GBJetInvtMass_UnitBinning->Sumw2();
  ...
  h_CSVM_GBJetInvtMass_binning40 = new TH1F("h_CSVM_GBJetInvtMass_binning40", "Invarient Mass Distribution of Gamma+CSVM-BJet", 125, 0.0, 5000.0);
  h_CSVM_GBJetInvtMass_binning40->GetYaxis()->SetTitle("Events/40 GeV");     h_CSVM_GBJetInvtMass_binning40->GetYaxis()->CenterTitle();
  h_CSVM_GBJetInvtMass_binning40->GetXaxis()->SetTitle("M({#gamma} + BJet)"); h_CSVM_GBJetInvtMass_binning40->GetXaxis()->CenterTitle();
  h_CSVM_GBJetInvtMass_binning40->Sumw2();

  h_CSVM_GBJetInvtMass_VariableBinning = new TH1F("h_CSVM_GBJetInvtMass_VariableBinning", "Invarient Mass Dist. of Gamma+CSVM-BJet", nbins, xbins);
  h_CSVM_GBJetInvtMass_VariableBinning->GetYaxis()->SetTitle("Events/Variable");     h_CSVM_GBJetInvtMass_VariableBinning->GetYaxis()->CenterTitle();
  h_CSVM_GBJetInvtMass_VariableBinning->GetXaxis()->SetTitle("M({#gamma} + BJet)"); h_CSVM_GBJetInvtMass_VariableBinning->GetXaxis()->CenterTitle();
  h_CSVM_GBJetInvtMass_VariableBinning->Sumw2();

  h_CSVM_GBJetInvtMass_UnitBinning = new TH1F("h_CSVM_GBJetInvtMass_UnitBinning", "Invarient Mass Dist. of Gamma+CSVM-BJet", 14000, 0, 14000);
  h_CSVM_GBJetInvtMass_UnitBinning->GetYaxis()->SetTitle("Events/UnitBin");     h_CSVM_GBJetInvtMass_UnitBinning->GetYaxis()->CenterTitle();
  h_CSVM_GBJetInvtMass_UnitBinning->GetXaxis()->SetTitle("M({#gamma} + BJet)");  h_CSVM_GBJetInvtMass_UnitBinning->GetXaxis()->CenterTitle();
  h_CSVM_GBJetInvtMass_UnitBinning->Sumw2();
  ...
  h_CSVT_GBJetInvtMass_binning40 = new TH1F("h_CSVT_GBJetInvtMass_binning40", "Invarient Mass Distribution of Gamma+CSVT-BJet", 125, 0.0, 5000.0);
  h_CSVT_GBJetInvtMass_binning40->GetYaxis()->SetTitle("Events/40 GeV");     h_CSVT_GBJetInvtMass_binning40->GetYaxis()->CenterTitle();
  h_CSVT_GBJetInvtMass_binning40->GetXaxis()->SetTitle("M({#gamma} + BJet)"); h_CSVT_GBJetInvtMass_binning40->GetXaxis()->CenterTitle();
  h_CSVT_GBJetInvtMass_binning40->Sumw2();

  h_CSVT_GBJetInvtMass_VariableBinning = new TH1F("h_CSVT_GBJetInvtMass_VariableBinning", "Invarient Mass Dist. of Gamma+CSVT-BJet", nbins, xbins);
  h_CSVT_GBJetInvtMass_VariableBinning->GetYaxis()->SetTitle("Events/Variable");     h_CSVT_GBJetInvtMass_VariableBinning->GetYaxis()->CenterTitle();
  h_CSVT_GBJetInvtMass_VariableBinning->GetXaxis()->SetTitle("M({#gamma} + BJet)"); h_CSVT_GBJetInvtMass_VariableBinning->GetXaxis()->CenterTitle();
  h_CSVT_GBJetInvtMass_VariableBinning->Sumw2();

  h_CSVT_GBJetInvtMass_UnitBinning = new TH1F("h_CSVT_GBJetInvtMass_UnitBinning", "Invarient Mass Dist. of Gamma+CSVT-BJet", 14000, 0, 14000);
  h_CSVT_GBJetInvtMass_UnitBinning->GetYaxis()->SetTitle("Events/UnitBin");     h_CSVT_GBJetInvtMass_UnitBinning->GetYaxis()->CenterTitle();
  h_CSVT_GBJetInvtMass_UnitBinning->GetXaxis()->SetTitle("M({#gamma} + BJet)");  h_CSVT_GBJetInvtMass_UnitBinning->GetXaxis()->CenterTitle();
  h_CSVT_GBJetInvtMass_UnitBinning->Sumw2();
  ...
  h_PassHLT150_PassProbes = new TH1F("h_PassHLT150_PassProbes", "Pt Distribution of Photons for events passing HLT_Photon150", 300, 0.0, 1500.0);
  h_PassHLT150_PassProbes->GetYaxis()->SetTitle("Events/5 GeV");         h_PassHLT150_PassProbes->GetYaxis()->CenterTitle();
  h_PassHLT150_PassProbes->GetXaxis()->SetTitle("P_{T}^{#gamma}");        h_PassHLT150_PassProbes->GetXaxis()->CenterTitle();
  h_PassHLT150_PassProbes->Sumw2();

  h_PassHLT75OR90_AllProbes = new TH1F("h_PassHLT75OR90_AllProbes", "Pt Distribution of Photons for events passing HLT_Photon75 OR HLT_Photon90", 300, 0.0, 1500.0);
  h_PassHLT75OR90_AllProbes->GetYaxis()->SetTitle("Events/5 GeV");         h_PassHLT75OR90_AllProbes->GetYaxis()->CenterTitle();
  h_PassHLT75OR90_AllProbes->GetXaxis()->SetTitle("P_{T}^{#gamma}");        h_PassHLT75OR90_AllProbes->GetXaxis()->CenterTitle();
  h_PassHLT75OR90_AllProbes->Sumw2();
