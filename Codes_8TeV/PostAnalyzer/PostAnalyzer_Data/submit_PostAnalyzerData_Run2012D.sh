...
sourceDir=/eos/uscms/store/user/rocky86/NTuples/Data/Data22Jan2013ReReco/Data_Run2012D_22Jan_AOD_NTuples/
...
tail -n +${sf} ${dataset} | head -n ${r} > Subset_Run2012D.txt
subdataset=Subset_Run2012D.txt
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for(Long64_t jentry = 0; jentry <nentries; jentry++){
	...
	if(PassCSVTBTag(i) == 1){
      ...
      //Filling histogram for BJetIdx vs BJetPt
      for(int i = 0; i < PFPatJet_n; i++){
	if(PassCSVLBTag(i) == 1){
          h_BJetIdxVsPt->Fill((*PFPatJet_pt)[i], i);
	}
      }
   ...
   Eff_CSVL = (double)CutFlowNumber[12]/(double)CutFlowNumber[11];
   Eff_CSVM = (double)CutFlowNumber[13]/(double)CutFlowNumber[11];
   Eff_CSVT = (double)CutFlowNumber[14]/(double)CutFlowNumber[11];

   Eff_Nodetadphicut_CSVL = (double)ncsvlbjets/(double)njets;
   Eff_Nodetadphicut_CSVM = (double)ncsvmbjets/(double)njets;
   Eff_Nodetadphicut_CSVT = (double)ncsvtbjets/(double)njets;
   ...
   cout << "****************************************************************************************************" << endl;
   cout << "Eff_CSVL = " << Eff_CSVL*100 << "%" << endl;
   cout << "Eff_CSVM = " << Eff_CSVM*100 << "%" << endl;
   cout << "Eff_CSVT = " << Eff_CSVT*100 << "%" << endl;
   cout << "****************************************************************************************************" << endl;
   cout << "Eff_Nodetadphicut_CSVL = " << Eff_Nodetadphicut_CSVL*100 << "%" << endl;
   cout << "Eff_Nodetadphicut_CSVM = " << Eff_Nodetadphicut_CSVM*100 << "%" << endl;
   cout << "Eff_Nodetadphicut_CSVT = " << Eff_Nodetadphicut_CSVT*100 << "%" << endl;
   cout << "****************************************************************************************************" << endl;
}
...
public :
   ...
   Double_t Eff_CSVL, Eff_CSVM, Eff_CSVT;
   Double_t Eff_Nodetadphicut_CSVL, Eff_Nodetadphicut_CSVM, Eff_Nodetadphicut_CSVT;
...
void PostAnalyzer_Data::BookHistograms(){
  ...
  h_GJetInvtMass = new TH1F("h_GJetInvtMass", "Invarient Mass Distribution of Gamma+Jet", 160, 0.0, 4000.0);
  h_GJetInvtMass->GetYaxis()->SetTitle("Events/25 GeV");     h_GJetInvtMass->GetYaxis()->CenterTitle();
  ...
  h_CSVL_BJetPhi = new TH1F("h_CSVL_BJetPhi", "Phi Distribution of BJets passing CSVL", 200, -5.0, 5.0);
  ...
  h_CSVL_GBJetInvtMass = new TH1F("h_CSVL_GBJetInvtMass", "Invarient Mass Distribution of Gamma+CSVL-BJet", 160, 0.0, 4000.0);
  h_CSVL_GBJetInvtMass->GetYaxis()->SetTitle("Events/25 GeV");     h_CSVL_GBJetInvtMass->GetYaxis()->CenterTitle();
  ...
  h_CSVM_BJetPhi = new TH1F("h_CSVM_BJetPhi", "Phi Distribution of BJets passing CSVM", 200, -5.0, 5.0);
  ...
  h_CSVM_GBJetInvtMass = new TH1F("h_CSVM_GBJetInvtMass", "Invarient Mass Distribution of Gamma+CSVM-BJet", 160, 0.0, 4000.0);
  h_CSVM_GBJetInvtMass->GetYaxis()->SetTitle("Events/25 GeV");     h_CSVM_GBJetInvtMass->GetYaxis()->CenterTitle();
  ...
  h_CSVT_BJetPhi = new TH1F("h_CSVT_BJetPhi", "Phi Distribution of BJets passing CSVT", 200, -5.0, 5.0);
  ...
  h_CSVT_GBJetInvtMass = new TH1F("h_CSVT_GBJetInvtMass", "Invarient Mass Distribution of Gamma+CSVT-BJet", 160, 0.0, 4000.0);
  h_CSVT_GBJetInvtMass->GetYaxis()->SetTitle("Events/25 GeV");     h_CSVT_GBJetInvtMass->GetYaxis()->CenterTitle();
  ...
  h_BJetIdxVsPt =new TH2F("h_BJetIdxVsPt", "Index of BJet in JetColl Vs. Pt of BJet", 100, 20.0, 2520.0, 100, 0, 200);
  h_BJetIdxVsPt->GetYaxis()->SetTitle("Idx Of BJet");             h_BJetIdxVsPt->GetYaxis()->CenterTitle();
  h_BJetIdxVsPt->GetXaxis()->SetTitle("BJet_Pt");                 h_BJetIdxVsPt->GetXaxis()->CenterTitle();
  h_BJetIdxVsPt->Sumw2();
