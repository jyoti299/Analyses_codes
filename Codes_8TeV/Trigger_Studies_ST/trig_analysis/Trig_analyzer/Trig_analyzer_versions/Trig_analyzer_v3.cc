...
Trig_analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(!triggerEventHandle_.isValid()){
    ...
    //-----------------------------------------------------------------------------
    //Getting leading and trailing leg filters of tnp_trigger 
    //Here last saved filter is for trailing leg and second last is for leading leg
    //However, this is not always true
    //------------------------------------------------------------------------------
    tnp_leadinglegfilter_ = *(tnp_filters_.rbegin() + 1); //rbegin is for reverse begin so rbegin + 1 = second last one
    tnp_trailinglegfilter_ = *(tnp_filters_.rbegin());
    ...
    //Getting the last filter as the other one is just saving L1 seeds
    std::string final_filter_Ele27_WP80 = *(filtersOf_Ele27_WP80_.rbegin());

    trigger::size_type Ele27_WP80_filterIdx = triggerEventHandle_->filterIndex(edm::InputTag(final_filter_Ele27_WP80, "", triggerEvent_.process()));
	...
	if((Invarient_m > 70 || Invarient_m == 70) && (Invarient_m < 120 || Invarient_m == 120)){
