// primary vertices : 1,2,3-4,5-100 are plotted. changes done by me. else if loop is considered under beginjob for primary vertices 1 and 2.
// the primary vertices range which earlier was PV=nPVBin[i]:nPVBin[i+1] is corrected to PV=nPVBin[i]:nPVBin[i+1]-1. 
#include "StudyHLT.h"
...
if (verbosity > 0) 
  ...
  bool ok(false);
	...
	//        const std::vector<std::string>& moduleLabels(hltConfig_.moduleLabels(triggerindx));
        int ipos=-1;
	std::string newtriggerName;
        newtriggerName = truncate_str(triggerNames_[iHLT]);
...
void StudyHLT::beginJob() {
  ...
  for (unsigned int i=0; i<8+nPVBin; i++) {
    if (i < 8) {
      ...
      sprintf(htit, "p_{T} of %s tracks (PV=%d)", TrkNames[7].c_str(), pvBin[i-8]);
    } else {
      sprintf(hname, "h_pt_%s_%d", TrkNames[7].c_str(), i-8);
    ...
    if (i < 8) {
      ...
      sprintf(htit, "Momentum of %s tracks (PV=%d)", TrkNames[7].c_str(), pvBin[i-8]);
    } else {
      sprintf(hname, "h_p_%s_%d", TrkNames[7].c_str(), i-8);
    ...
    if (i < 8) {
      ...
      sprintf(htit, "Eta of %s tracks (PV=%d)", TrkNames[7].c_str(), pvBin[i-8]);
    } else {
      sprintf(hname, "h_eta_%s_%d", TrkNames[7].c_str(), i-8);
    ...
    if (i < 8) {
      ...
      sprintf(hname, "h_phi%s_%d", TrkNames[7].c_str(), i-8);
      sprintf(htit, "Phi of %s tracks (PV=%d)", TrkNames[7].c_str(), pvBin[i-8]);
    } else {
  ...
  for (int i=0; i<4+nPVBin; ++i) {
	...
	for (int j=0; j<6; ++j) {
	  ...
	  if (i < 4) {
	    ...
	    sprintf(htit,"%s/p (p=%4.1f:%4.1f; i#eta=%d:%d, PV=%d) for tracks with %s",
                    energyNames[j].c_str(),pBin[ip],pBin[ip+1],etaBin[ie],
                    (etaBin[ie+1]-1), pvBin[i-4], TrkNames[7].c_str());
          } else {
	    sprintf(htit,"%s/p (p=%4.1f:%4.1f; i#eta=%d:%d, PV=%d:%d) for tracks with %s",
		    energyNames[j].c_str(),pBin[ip],pBin[ip+1],etaBin[ie],
		    (etaBin[ie+1]-1), pvBin[i-4], pvBin[i-3]-1,
		    TrkNames[7].c_str());
	  }
