// for different ranges of good primary vertices (1-2,3-4,5-100) are considered as well as truncate_str function is included
...
if (verbosity > 0) 
  ...
  bool ok(false);
     if (trigNames.size() < 1) {                                         //    This can as well be used -- if (trigNames[0]=="NULL"){ by putting TrigNames = "NULL" in cfg.py
    ok = true;
...
void StudyHLT::beginJob() {
  ...
  for (unsigned int i=0; i<8+nPVBin; i++) {
    if (i < 8) {
      ...
      sprintf(htit, "p_{T} of %s tracks (PV=%d-%d)", TrkNames[7].c_str(), pvBin[i-8], pvBin[i-7]);
    }
    ...
    if (i < 8) {
      ...
      sprintf(htit, "Momentum of %s tracks (PV=%d-%d)", TrkNames[7].c_str(), pvBin[i-8], pvBin[i-7]);
    }
    ...
    if (i < 8) {
      ...
      sprintf(htit, "Eta of %s tracks (PV=%d-%d)", TrkNames[7].c_str(), pvBin[i-8], pvBin[i-7]);
    }
    ...
    if (i < 8) {
      ...
      sprintf(htit, "Phi of %s tracks (PV=%d-%d)", TrkNames[7].c_str(), pvBin[i-8], pvBin[i-7]);
    }
...
std::string StudyHLT::truncate_str(std::string str){
  int i=0,z=0,length;
