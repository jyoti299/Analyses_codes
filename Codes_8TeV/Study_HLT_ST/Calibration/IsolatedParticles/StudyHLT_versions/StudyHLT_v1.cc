// Initial code, on consideration on primary vertices as well as truncate_str function not included so is a problem within this loop:for (unsigned int iHLT=0; iHLT<triggerResults->size(); iHLT++) {
//code shows an error while going from one run to the next in the data. The reason is the use of triggers of different versions in different runs so that the code under the above loop causes problem.
...
void StudyHLT::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (Lumid.isValid()) {
    ...
    std::cout << "LUMIPRODUCER NOT FOUND" << std::endl;  
  }
  if (verbosity > 0)
  ...
  if (trigNames.size() < 1) {
	...
	for (int i=0; i<5; ++i) {
	  if (triggerNames_[iHLT].find(newNames[i].c_str())!=std::string::npos) {
	    if (verbosity%10 > 0)
	      std::cout << "[" << i << "] " << newNames[i] << " : " 
			<< triggerNames_[iHLT] << std::endl;
  ...
  if (ok) {
	...
	if (trkDetItr->okHCAL) {
	  ...
	  if ((verbosity/100000)%10 > 0 && firstPrint) {
	    firstPrint = false;
