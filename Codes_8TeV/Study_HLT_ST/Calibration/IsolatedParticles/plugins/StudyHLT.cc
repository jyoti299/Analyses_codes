...
if (verbosity > 0) 
  ...
  if (trigNames.size() < 1) {
	...
	iflg += (indx*newAccept[i]); indx *= 2;
      ...
      //Here the correlation is defined such that in newAccept[i], for uncorrlated events i.e events passing only one type of trigger, only one 
      //(for HLT) and two (HLT + pixel/zerobias/physics/jetE) columns will be one and others zero. And for correlated events, there will be more
      //than two columns that would be one. so for uncorrelated events the possible combinations are [10000] [11000],[10100],[10010],[10001] which 
      //corresponds to iflg = 1,3,5,9 and 17 respectively. So if events corresponding to these bins are filled, these will be non correlated events.
      // And for correlated events, the possible combinations of newAccept[i] are [11100], [11010], [10110], [11001], [10101]and [10011] which 
      //corresponds to iflg values of 7, 11, 13, 19, 21 and 25. So if the events for these bins will be filled then those will be correlated events 
      //i.e events satisfying more than one type of triggers. 
  ...
  if (ok) {
	...
	////maxNearP31x31 gives the track of maximum energy among the tracks that are in 31x31 of the selected track. This we do by propogating all the tracks upto the calorimeter surface by applying a loose criteria and seeing the energy deposits by them. 
...
void StudyHLT::beginJob() {
  ...
  h_goodRun     = fs->make<TH1I>("h_goodRun","Number of accepted events for Run", 4000, 190000, 194000);
